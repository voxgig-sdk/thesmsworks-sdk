...
class TestBatchDirect:
    ...
    def test_should_direct_load_batch(self):
        setup = _batch_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-batch", "live" if setup["live"] else "unit")
        if _skip:
...
def _batch_direct_setup(mockres):
