...
class ThesmsworksTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ThesmsworksTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ThesmsworksTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ThesmsworksTestRunner.getenv("THESMSWORKS_TEST_LIVE")
        override = ThesmsworksTestRunner.getenv("THESMSWORKS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ThesmsworksTestRunner.getenv(key)
        ...
        explain = ThesmsworksTestRunner.getenv("THESMSWORKS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["THESMSWORKS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ThesmsworksTestRunner._test_control is not None:
            return ThesmsworksTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ThesmsworksTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ThesmsworksTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ThesmsworksTestRunner.load_test_control()
...
def env_override(m):
    return ThesmsworksTestRunner.env_override(m)


def entity_data(v):
    return ThesmsworksTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ThesmsworksTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ThesmsworksTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ThesmsworksTestRunner.load_test_control()


def live_client_options():
    return ThesmsworksTestRunner.live_client_options()


def live_delay_ms():
    return ThesmsworksTestRunner.live_delay_ms()
