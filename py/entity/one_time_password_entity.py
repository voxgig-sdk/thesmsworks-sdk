...
class OneTimePasswordEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return OneTimePasswordEntity(self._client, opts)
    ...
    def data_get(self) -> OneTimePassword:
    ...
    def match_get(self) -> OneTimePassword:
        self._utility.feature_hook(self._entctx, "GetMatch")
        return vs.clone(self._match)

    
    def load(self, reqmatch=None, ctrl=None) -> OneTimePassword:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.OneTimePassword().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: OneTimePasswordCreateData, ctrl=None) -> OneTimePassword:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
