...
class BatchMessageEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BatchMessageEntity(self._client, opts)
    ...
    def data_get(self) -> BatchMessage:
    ...
    def match_get(self) -> BatchMessage:
    ...
    def create(self, reqdata: BatchMessageCreateData, ctrl=None) -> BatchMessage:
        utility = self._utility
    ...
    def remove(self, reqmatch=None, ctrl=None) -> BatchMessage:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.BatchMessage().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
