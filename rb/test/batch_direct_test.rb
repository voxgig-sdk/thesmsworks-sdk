...
class BatchDirectTest < Minitest::Test
  def test_direct_load_batch
    setup = batch_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-batch", setup[:live] ? "live" : "unit")
    if _should_skip
...
def batch_direct_setup(mockres)
