...
class BatchEntity
  ...
  # @return [Batch, Hash] the current Batch data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Batch fields)
  def match_get
  ...
  # Load a single Batch.
  #
  # @param reqmatch [BatchLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Batch.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Batch, Hash] the loaded Batch; raises ThesmsworksError on failure
