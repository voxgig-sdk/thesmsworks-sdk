...
boru-driven command-line client **and** interactive REPL for the Thesmsworks
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/thesmsworks-cli)
make build

# 2. See usage (words, entities, env vars)
./thesmsworks-cli --help

# 3. Provide credentials once, via the environment
export THESMSWORKS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./thesmsworks-cli load 1 batch            # {id:1} shorthand
./thesmsworks-cli load '{id:1}' batch       # explicit match map

# 5. Override the API base URL for a single call
THESMSWORKS_BASE=https://api.example.com ./thesmsworks-cli load 1 batch

# 6. No arguments -> interactive REPL
./thesmsworks-cli
thesmsworks> load 1 batch
thesmsworks> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/thesmsworks-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/thesmsworks-cli load 1 batch
   ```
...
```sh
./thesmsworks-cli load 1 batch          # scalar shorthand for {id:1}
./thesmsworks-cli load '{id:1}' batch     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `thesmsworks>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `batch`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/thesmsworks-cli`. |
...
(`./thesmsworks-cli <expr>`) and interactively (the REPL), and expressions compose the
