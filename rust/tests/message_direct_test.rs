// Generated direct-call tests for the message entity (mirrors the
...
fn message_direct_setup(mockres: Value) -> MessageDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("THESMSWORKS_TEST_MESSAGE_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "THESMSWORKS_TEST_MESSAGE_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return MessageDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn message_direct_load() {
    let setup = message_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-message", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        setp(&params, "id", getp(&setup.idmap, "message01"));
    } else {
