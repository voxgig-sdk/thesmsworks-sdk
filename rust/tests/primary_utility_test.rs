...
use thesmsworks_sdk::core::helpers::{get_str, getp, ja, jo, setp, to_map, vfn};
...
use thesmsworks_sdk::{
    ...
    ThesmsworksError, ThesmsworksSDK, SdkResult, Spec, Utility, Value,
};
...
where
    F: FnMut(&mut Vec<Value>) -> Result<Value, ThesmsworksError> + 'static,
{
...
fn base_client() -> (Rc<ThesmsworksSDK>, Rc<Utility>) {
...
fn make_test_ctx(client: &Rc<ThesmsworksSDK>, utility: &Rc<Utility>) -> Rc<Context> {
...
fn make_test_full_ctx(client: &Rc<ThesmsworksSDK>, utility: &Rc<Utility>) -> Rc<Context> {
...
fn primary_fetcher_live() {
    ...
    let c = calls.clone();
    let live_client = ThesmsworksSDK::new(jo(vec![(
        "system",
        jo(vec![(
            "fetch",
            vfn(move |args| {
...
fn primary_fetcher_blocked_test_mode() {
    ...
    let blocked_client = ThesmsworksSDK::new(jo(vec![(
        "system",
