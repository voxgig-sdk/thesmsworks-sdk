// Generated direct-call tests for the one_time_password entity (mirrors the
...
fn one_time_password_direct_setup(mockres: Value) -> OneTimePasswordDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("THESMSWORKS_TEST_ONE_TIME_PASSWORD_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "THESMSWORKS_TEST_ONE_TIME_PASSWORD_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return OneTimePasswordDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn one_time_password_direct_load() {
    let setup = one_time_password_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-one_time_password", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        setp(&params, "messageid", getp(&setup.idmap, "messageid01"));
    } else {
        setp(&params, "messageid", Value::str("direct01"));
    }

    let result = client
        .direct(jo(vec![
            ("path", Value::str("otp/{messageid}")),
