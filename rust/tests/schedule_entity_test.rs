...
#[test]
fn schedule_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.schedule(Value::Noval);
    assert_eq!(ent.get_name(), "schedule");
}
...
fn schedule_entity_basic() {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set THESMSWORKS_TEST_SCHEDULE_ENTID JSON to run live");
        return;
    }

    // Bootstrap entity data from existing test data (no create step in flow).
    let schedule_ref01_data_raw = vs::items(&to_map(&getpath(&["existing", "schedule"], &setup.data)));
    let schedule_ref01_data = to_map(&vs::get_elem(
        &vs::get_elem(&schedule_ref01_data_raw, &Value::Num(0.0), Value::Noval),
        &Value::Num(1.0),
        Value::Noval,
    ));
}

fn schedule_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("schedule");
    entity_data_file.push("ScheduleTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("schedule01"), Value::str("schedule02"), Value::str("schedule03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("THESMSWORKS_TEST_SCHEDULE_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("THESMSWORKS_TEST_SCHEDULE_ENTID", idmap.clone()),
        ("THESMSWORKS_TEST_LIVE", Value::str("FALSE")),
        ("THESMSWORKS_TEST_EXPLAIN", Value::str("FALSE")),
        ("THESMSWORKS_APIKEY", Value::str("NONE")),
    ]));

    let idmap_resolved = match to_map(&getp(&env, "THESMSWORKS_TEST_SCHEDULE_ENTID")) {
