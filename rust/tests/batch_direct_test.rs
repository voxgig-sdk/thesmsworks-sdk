// Generated direct-call tests for the batch entity (mirrors the
...
fn batch_direct_setup(mockres: Value) -> BatchDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("THESMSWORKS_TEST_BATCH_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "THESMSWORKS_TEST_BATCH_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return BatchDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn batch_direct_load() {
    let setup = batch_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-batch", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        setp(&params, "id", getp(&setup.idmap, "batch01"));
    } else {
