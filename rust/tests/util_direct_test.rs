// Generated direct-call tests for the util entity (mirrors the
...
fn util_direct_setup(mockres: Value) -> UtilDirectSetup {
    load_env_local();
...
#[test]
fn util_direct_load() {
    let setup = util_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-util", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        setp(&params, "errorcode", getp(&setup.idmap, "errorcode01"));
    } else {
        setp(&params, "errorcode", Value::str("direct01"));
    }

    let result = client
        .direct(jo(vec![
            ("path", Value::str("utils/errors/{errorcode}")),
