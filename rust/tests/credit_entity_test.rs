...
#[test]
fn credit_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.credit(Value::Noval);
    assert_eq!(ent.get_name(), "credit");
}

#[test]
fn credit_entity_stream() {
    ...
    let plainent = plainsdk.credit(Value::Noval);
...
#[test]
fn credit_entity_basic() {
    let setup = credit_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["load"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("credit.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set THESMSWORKS_TEST_CREDIT_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();

    // Bootstrap entity data from existing test data (no create step in flow).
    let credit_ref01_data_raw = vs::items(&to_map(&getpath(&["existing", "credit"], &setup.data)));
    let credit_ref01_data = to_map(&vs::get_elem(
        &vs::get_elem(&credit_ref01_data_raw, &Value::Num(0.0), Value::Noval),
        &Value::Num(1.0),
        Value::Noval,
    ));
    // LOAD
    let credit_ref01_ent = client.credit(Value::Noval);
    let credit_ref01_match_dt0 = Value::empty_map();
    let credit_ref01_data_dt0_loaded = credit_ref01_ent
        .load(credit_ref01_match_dt0.clone(), Value::Noval)
        .expect("load failed");
    assert!(
        !credit_ref01_data_dt0_loaded.is_noval(),
        "expected load result to be non-nil"
    );

}

fn credit_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("credit");
    entity_data_file.push("CreditTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("credit01"), Value::str("credit02"), Value::str("credit03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("THESMSWORKS_TEST_CREDIT_ENTID").unwrap_or_default();
