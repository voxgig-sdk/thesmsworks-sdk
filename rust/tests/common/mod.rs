...
use thesmsworks_sdk::core::helpers::{
...
use thesmsworks_sdk::{
    ...
    ThesmsworksError, ThesmsworksSDK, Response, SdkResult, Spec, Utility, Value,
};
...
/// env_override (mirrors go): when THESMSWORKS_TEST_LIVE or
/// THESMSWORKS_TEST_OVERRIDE is TRUE, environment variables replace the
/// defaults in `m` (JSON-shaped values are parsed).
pub fn env_override(m: Value) -> Value {
    let live = std::env::var("THESMSWORKS_TEST_LIVE").unwrap_or_default() == "TRUE";
    let over = std::env::var("THESMSWORKS_TEST_OVERRIDE").unwrap_or_default() == "TRUE";
    ...
    if let Ok(explain) = std::env::var("THESMSWORKS_TEST_EXPLAIN") {
        if !explain.is_empty() {
            setp(&m, "THESMSWORKS_TEST_EXPLAIN", Value::str(explain));
        }
    }
...
pub fn match_string(pattern: &str, val: &str) -> bool {
    if pattern.len() >= 2 && pattern.starts_with('/') && pattern.ends_with('/') {
        return thesmsworks_sdk::utility::voxgigstruct::re::Regex::new(
    ...
    if let Value::Map(_) = getp(&ctxmap, "result") {
        ...
        if let Value::Map(_) = getp(&res_map, "err") {
            if let Some(msg) = get_str(&getp(&res_map, "err"), "message") {
                result.borrow_mut().err = Some(ThesmsworksError::new("", &msg));
            }
        }
...
pub fn fixctx(ctx: &Rc<Context>, client: &Rc<ThesmsworksSDK>) {
    if ctx.options.borrow().is_noval() {
        *ctx.options.borrow_mut() = client.options_map();
    }
}
...
pub fn err_from_map(m: &Value) -> Option<ThesmsworksError> {
    let msg = get_str(m, "message").unwrap_or_default();
    if msg.is_empty() {
        return None;
    }
    let code = get_str(m, "code").unwrap_or_default();
    Some(ThesmsworksError::new(&code, &msg))
}
...
pub fn fh_has_feature(name: &str) -> bool {
    let config = thesmsworks_sdk::make_config();
...
pub type FhReply = Rc<dyn Fn(i64, &Value) -> Result<Value, ThesmsworksError>>;
...
pub struct FhOpResult {
    pub ok: bool,
    pub data: Value,
    pub err: Option<ThesmsworksError>,
...
fn fh_populate_result(ctx: &Rc<Context>, fetched: &Result<Value, ThesmsworksError>) {
    ...
    *ctx.result.borrow_mut() = Some(result.clone());
    ...
    let resp = Response::new(response_map);
    {
        ...
        if let Value::Func(_) = &resp.json {
            r.body = thesmsworks_sdk::call_json(&resp.json);
        }
...
impl FhHarness {
    ...
    pub fn op(&self, o: FhOpSpec) -> FhOpResult {
        ...
        *ctx.spec.borrow_mut() = Some(spec.clone());
        ...
        let fetched: Result<Value, ThesmsworksError> = match ctx.out_get("request") {
    ...
    fn fail(&self, ctx: Rc<Context>, err: ThesmsworksError) -> FhOpResult {
        {
...
pub fn fh_err_code(err: &Option<ThesmsworksError>) -> String {
