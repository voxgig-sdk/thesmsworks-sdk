...
use thesmsworks_sdk::core::helpers::{get_bool, get_str, getp, ja, jo, vfn};
use thesmsworks_sdk::feature::audit::AuditFeature;
use thesmsworks_sdk::feature::cache::CacheFeature;
use thesmsworks_sdk::feature::clienttrack::ClienttrackFeature;
use thesmsworks_sdk::feature::debug::DebugFeature;
use thesmsworks_sdk::feature::idempotency::IdempotencyFeature;
use thesmsworks_sdk::feature::metrics::MetricsFeature;
use thesmsworks_sdk::feature::netsim::NetsimFeature;
use thesmsworks_sdk::feature::paging::PagingFeature;
use thesmsworks_sdk::feature::proxy::ProxyFeature;
use thesmsworks_sdk::feature::ratelimit::RatelimitFeature;
use thesmsworks_sdk::feature::rbac::RbacFeature;
use thesmsworks_sdk::feature::retry::RetryFeature;
use thesmsworks_sdk::feature::streaming::StreamingFeature;
use thesmsworks_sdk::feature::telemetry::TelemetryFeature;
use thesmsworks_sdk::feature::timeout::TimeoutFeature;
use thesmsworks_sdk::utility::voxgigstruct as vs;
use thesmsworks_sdk::{Context, FeatureRef, Value};

fn fr<T: thesmsworks_sdk::Feature + 'static>(f: &Rc<RefCell<T>>) -> FeatureRef {
...
fn feature_retry_retries_transport_error_then_returns_it() {
    ...
    let nc = n.clone();
    let server: thesmsworks_sdk::FetcherFn = Rc::new(move |ctx: &Rc<Context>, _u, _f| {
        *nc.borrow_mut() += 1;
...
fn feature_retry_retries_nil_transport_result() {
    ...
    let n = Rc::new(RefCell::new(0));
    let nc = n.clone();
    let server: thesmsworks_sdk::FetcherFn = Rc::new(move |_c: &Rc<Context>, _u, _f| {
        *nc.borrow_mut() += 1;
        if *nc.borrow() < 2 {
            Ok(Value::Noval)
        } else {
...
fn feature_timeout_slow_request_times_out() {
    ...
    let server: thesmsworks_sdk::FetcherFn = Rc::new(|_c: &Rc<Context>, _u, _f| {
