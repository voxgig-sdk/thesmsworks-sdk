...
#[test]
fn util_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.util(Value::Noval);
    assert_eq!(ent.get_name(), "util");
}
...
fn util_entity_basic() {
    ...
    for op in ["load"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("util.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set THESMSWORKS_TEST_UTIL_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();

    // Bootstrap entity data from existing test data (no create step in flow).
    let util_ref01_data_raw = vs::items(&to_map(&getpath(&["existing", "util"], &setup.data)));
    let util_ref01_data = to_map(&vs::get_elem(
        &vs::get_elem(&util_ref01_data_raw, &Value::Num(0.0), Value::Noval),
        &Value::Num(1.0),
        Value::Noval,
    ));
    // LOAD
    let util_ref01_ent = client.util(Value::Noval);
    let util_ref01_match_dt0 = Value::empty_map();
...
fn util_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("util");
    entity_data_file.push("UtilTestData.json");
    ...
    let entid_env_raw = std::env::var("THESMSWORKS_TEST_UTIL_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("THESMSWORKS_TEST_UTIL_ENTID", idmap.clone()),
        ("THESMSWORKS_TEST_LIVE", Value::str("FALSE")),
        ("THESMSWORKS_TEST_EXPLAIN", Value::str("FALSE")),
        ("THESMSWORKS_APIKEY", Value::str("NONE")),
    ]));

    let idmap_resolved = match to_map(&getp(&env, "THESMSWORKS_TEST_UTIL_ENTID")) {
