...
local BatchMessageEntity = {}
BatchMessageEntity.__index = BatchMessageEntity


function BatchMessageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BatchMessageEntity)
  self._name = "batch_message"
  self._client = client
...
function BatchMessageEntity:get_name()
  return self._name
end


function BatchMessageEntity:make()
  local opts = {}
...
function BatchMessageEntity:mark_deleted()
  self._deleted = true
end


function BatchMessageEntity:deleted()
  return true == self._deleted
end


function BatchMessageEntity:data_set(args)
  if args ~= nil then
...
function BatchMessageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BatchMessageEntity:match_set(args)
  if args ~= nil then
...
function BatchMessageEntity:match_get()
...
function BatchMessageEntity:stream(action, args, callopts)
...
---@param reqdata BatchMessageCreateData
---@param ctrl? table
---@return BatchMessage
---@return string? err
function BatchMessageEntity:create(reqdata, ctrl)
  local utility = self._utility
...
---@param reqmatch BatchMessageRemoveMatch
---@param ctrl? table
---@return BatchMessage
---@return string? err
function BatchMessageEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function BatchMessageEntity:_run_op(ctx, post_done)
  local utility = self._utility
