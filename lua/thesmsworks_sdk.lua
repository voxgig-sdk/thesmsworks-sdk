...
function ThesmsworksSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "ThesmsworksSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function ThesmsworksSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ThesmsworksSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ThesmsworksSDK: graphql: " .. msg
    res.graphql = errors
  end
