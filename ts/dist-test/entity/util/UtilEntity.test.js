...
(0, node_test_1.describe)('UtilEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'util.' + op, live))
                return;
        }
...
function basicSetup(extra) {
    ...
    let idmap = transform(['util01', 'util02', 'util03', 'error01', 'error02', 'error03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['THESMSWORKS_TEST_UTIL_ENTID'];
