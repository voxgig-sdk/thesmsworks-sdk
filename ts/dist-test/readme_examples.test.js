...
const TEST_SEED = { "entity": { "batch": { "test01": { "id": "test01" } }, "batch_message": { "test01": { "id": "test01" } }, "credit": { "test01": { "id": "test01" } }, "flash": { "test01": { "id": "test01" } }, "message": { "test01": { "id": "test01" } }, "one_time_password": { "test01": { "id": "test01" } }, "schedule": { "test01": { "id": "test01" } }, "swagger": { "test01": { "id": "test01" } }, "util": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+ThesmsworksSDK\b/.test(code) ||
        /\bThesmsworksSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+ThesmsworksSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/ThesmsworksSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
