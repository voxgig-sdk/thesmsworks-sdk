...
describe('OneTimePasswordDirect', async () => {
  ...
  test('direct-load-one_time_password', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-one_time_password', setup.live)) return
    if (skipIfMissingIds(t, setup, ["messageid01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['THESMSWORKS_TEST_ONE_TIME_PASSWORD_ENTID']
