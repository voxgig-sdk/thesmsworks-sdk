...
describe('UtilDirect', async () => {
  ...
  test('direct-load-util', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-util', setup.live)) return
    if (skipIfMissingIds(t, setup, ["errorcode01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['THESMSWORKS_TEST_UTIL_ENTID']
