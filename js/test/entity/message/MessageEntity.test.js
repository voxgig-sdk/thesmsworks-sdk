...
describe('MessageEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        message: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.Message().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.Message().stream('list')) {
  ...
  test('basic', async () => {
    ...
    assert(null != message_ref01_data_dt0)
