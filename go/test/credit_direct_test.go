...
func TestCreditDirect(t *testing.T) {
	t.Run("direct-load-credit", func(t *testing.T) {
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-credit", _mode); _shouldSkip {
			if _reason == "" {
...
type creditDirectSetupResult struct {
...
func creditDirectSetup(mockres any) *creditDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"THESMSWORKS_TEST_CREDIT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["THESMSWORKS_TEST_CREDIT_ENTID"]; ok {
		...
		return &creditDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &creditDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
