...
import (
	...
	omni "github.com/voxgig-sdk/thesmsworks-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ThesmsworksError); is {
				out := map[string]any{
					"name":    "ThesmsworksError",
...
func omniCtx(arg any, client *sdk.ThesmsworksSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ThesmsworksSDK); is {
			client = live
