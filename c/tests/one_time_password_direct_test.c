// Generated direct-call test for the one_time_password entity (mirrors the
...
static voxgig_value* one_time_password_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static ThesmsworksSDK* one_time_password_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(one_time_password_mock, mockres)));
...
int main(void) {
  ...
  // LOAD
  {
    ...
    ThesmsworksSDK* sdk = one_time_password_direct_setup(mockres);
    voxgig_value* params = v_map();
    setp(params, "messageid", v_str("direct01"));
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("otp/{messageid}"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  TEST_SUMMARY("thesmsworks_one_time_password_direct");
}
