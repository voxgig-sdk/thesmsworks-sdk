// Generated direct-call test for the batch entity (mirrors the
...
static voxgig_value* batch_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static ThesmsworksSDK* batch_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(batch_mock, mockres)));
...
int main(void) {
  ...
  // LOAD
  {
    ...
    ThesmsworksSDK* sdk = batch_direct_setup(mockres);
  ...
  TEST_SUMMARY("thesmsworks_batch_direct");
}
