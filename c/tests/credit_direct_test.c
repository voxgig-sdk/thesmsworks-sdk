// Generated direct-call test for the credit entity (mirrors the
...
static voxgig_value* credit_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static ThesmsworksSDK* credit_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(credit_mock, mockres)));
...
int main(void) {
  ...
  // LOAD
  {
    ...
    ThesmsworksSDK* sdk = credit_direct_setup(mockres);
    voxgig_value* params = v_map();
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("credits/balance"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  TEST_SUMMARY("thesmsworks_credit_direct");
}
