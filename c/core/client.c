// ThesmsworksSDK client (generated — mirrors the rust Main fragment).
...
ThesmsworksSDK* thesmsworks_sdk_new(voxgig_value* options) {
  ThesmsworksSDK* sdk = (ThesmsworksSDK*)calloc(1, sizeof(ThesmsworksSDK));
...
static bool sdk_op_allowed(ThesmsworksSDK* sdk, const char* op) {
...
static voxgig_value* sdk_op_denied(ThesmsworksSDK* sdk, const char* op) {
  voxgig_value* allow_op = getpath2(sdk->options, "allow", "op");
  const char* allow = voxgig_is_string(allow_op) ? voxgig_as_string(allow_op) : "";
  char msg[512];
  snprintf(msg, sizeof(msg),
    "ThesmsworksSDK: %s: operation not allowed by"
...
static voxgig_value* sdk_raw_request(
  ThesmsworksSDK* sdk, voxgig_value* fetchargs, PNError** err) {
  *err = NULL;
  Utility* utility = sdk->utility;
...
voxgig_value* sdk_graphql(ThesmsworksSDK* sdk, const char* query,
  ...
  *err = NULL;
  ...
  if (voxgig_is_list(errors) && 0 < voxgig_as_list(errors)->len) {
    ...
    char msg[512];
    snprintf(msg, sizeof(msg), "ThesmsworksSDK: graphql: %s", m);
...
Entity* thesmsworks_util(ThesmsworksSDK* client, voxgig_value* entopts) {
  return util_entity_new(client, entopts);
}
...
ThesmsworksSDK* test_sdk(voxgig_value* testopts, voxgig_value* sdkopts) {
  ...
  ThesmsworksSDK* sdk = thesmsworks_sdk_new(sdkopts);
  free(sdk->mode);
  sdk->mode = strdup("test");
  return sdk;
}
