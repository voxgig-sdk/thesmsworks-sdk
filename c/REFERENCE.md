...
| `sender` | `char*` | No | The sender of the message (this can be the configured sender name for an outbound message or the senders phone number for an inbound message). |
...
```c
Entity* message = thesmsworks_message(client, NULL);
voxgig_value* result = message->vt->create(message, NULL, NULL, &err);
```
