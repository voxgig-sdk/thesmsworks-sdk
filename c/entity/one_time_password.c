// OneTimePassword entity client (generated — mirrors the rust Entity fragment).
...
typedef struct one_time_password_entity {
...
static const char* one_time_password_get_name(Entity* e);
...
Entity* one_time_password_entity_new(ThesmsworksSDK* client, voxgig_value* entopts) {
  ...
  one_time_password_entity* self = (one_time_password_entity*)calloc(1, sizeof(one_time_password_entity));
  self->base.vt = &one_time_password_VT;
  self->name = strdup("one_time_password");
  self->client = client;
...
static voxgig_value* one_time_password_run_op(one_time_password_entity* self, Context* ctx,
                                    one_time_password_postdone_fn post_done, PNError** err) {
...
voxgig_value* one_time_password_stream(Entity* e, const char* action, voxgig_value* args,
...
static const char* one_time_password_get_name(Entity* e) {
  return ((one_time_password_entity*)e)->name;
}
...
static Entity* one_time_password_make(Entity* e) {
  ...
  return one_time_password_entity_new(self->client, opts);
}
...
static voxgig_value* one_time_password_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(one_time_password_ent_ctx(self), "SetData");
  }
  feature_hook_util(one_time_password_ent_ctx(self), "GetData");
...
static voxgig_value* one_time_password_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(one_time_password_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(one_time_password_ent_ctx(self), "GetMatch");
  return voxgig_clone(self->mtch);
}


static void one_time_password_load_postdone(one_time_password_entity* self, Context* ctx) {
...
static void one_time_password_create_postdone(one_time_password_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resdata = result->resdata;
    if (!v_is_noval(resdata) && !v_is_null(resdata)) {
...
static const EntityVT one_time_password_VT = {
  ...
  one_time_password_update,
  one_time_password_remove,
};
