// Schedule entity client (generated — mirrors the rust Entity fragment).
...
typedef struct schedule_entity {
...
static const char* schedule_get_name(Entity* e);
...
Entity* schedule_entity_new(ThesmsworksSDK* client, voxgig_value* entopts) {
  ...
  schedule_entity* self = (schedule_entity*)calloc(1, sizeof(schedule_entity));
  self->base.vt = &schedule_VT;
  self->name = strdup("schedule");
  self->client = client;
...
static voxgig_value* schedule_run_op(schedule_entity* self, Context* ctx,
                                    schedule_postdone_fn post_done, PNError** err) {
...
voxgig_value* schedule_stream(Entity* e, const char* action, voxgig_value* args,
                             voxgig_value* callopts, PNError** err) {
  schedule_entity* self = (schedule_entity*)e;
  *err = NULL;
  ...
  Context* ctx = make_context_util(cs, schedule_ent_ctx(self));
...
static Entity* schedule_make(Entity* e) {
  ...
  return schedule_entity_new(self->client, opts);
}
...
static voxgig_value* schedule_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(schedule_ent_ctx(self), "SetData");
  }
  feature_hook_util(schedule_ent_ctx(self), "GetData");
...
static voxgig_value* schedule_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(schedule_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(schedule_ent_ctx(self), "GetMatch");
