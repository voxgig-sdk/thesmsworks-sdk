// Message entity client (generated — mirrors the rust Entity fragment).
...
static const char* message_get_name(Entity* e);
...
Entity* message_entity_new(ThesmsworksSDK* client, voxgig_value* entopts) {
  ...
  message_entity* self = (message_entity*)calloc(1, sizeof(message_entity));
  self->base.vt = &message_VT;
  self->name = strdup("message");
  self->client = client;
...
static voxgig_value* message_run_op(message_entity* self, Context* ctx,
                                    message_postdone_fn post_done, PNError** err) {
...
voxgig_value* message_stream(Entity* e, const char* action, voxgig_value* args,
...
static Entity* message_make(Entity* e) {
  ...
  return message_entity_new(self->client, opts);
}
...
static voxgig_value* message_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(message_ent_ctx(self), "SetData");
  }
  feature_hook_util(message_ent_ctx(self), "GetData");
...
static voxgig_value* message_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(message_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(message_ent_ctx(self), "GetMatch");
  return voxgig_clone(self->mtch);
}


static void message_load_postdone(message_entity* self, Context* ctx) {
...
static void message_create_postdone(message_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resdata = result->resdata;
    if (!v_is_noval(resdata) && !v_is_null(resdata)) {
...
static void message_remove_postdone(message_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resmatch = result->resmatch;
    voxgig_value* resdata = result->resdata;
