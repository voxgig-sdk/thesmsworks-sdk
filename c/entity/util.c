// Util entity client (generated — mirrors the rust Entity fragment).
...
static const char* util_get_name(Entity* e);
...
Entity* util_entity_new(ThesmsworksSDK* client, voxgig_value* entopts) {
  ...
  util_entity* self = (util_entity*)calloc(1, sizeof(util_entity));
  self->base.vt = &util_VT;
  self->name = strdup("util");
  self->client = client;
...
static voxgig_value* util_run_op(util_entity* self, Context* ctx,
                                    util_postdone_fn post_done, PNError** err) {
...
voxgig_value* util_stream(Entity* e, const char* action, voxgig_value* args,
...
static Entity* util_make(Entity* e) {
  ...
  return util_entity_new(self->client, opts);
}
...
static voxgig_value* util_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(util_ent_ctx(self), "SetData");
  }
  feature_hook_util(util_ent_ctx(self), "GetData");
...
static voxgig_value* util_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(util_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(util_ent_ctx(self), "GetMatch");
  return voxgig_clone(self->mtch);
}


static void util_load_postdone(util_entity* self, Context* ctx) {
