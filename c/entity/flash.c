// Flash entity client (generated — mirrors the rust Entity fragment).
...
static const char* flash_get_name(Entity* e);
...
Entity* flash_entity_new(ThesmsworksSDK* client, voxgig_value* entopts) {
  ...
  flash_entity* self = (flash_entity*)calloc(1, sizeof(flash_entity));
  self->base.vt = &flash_VT;
  self->name = strdup("flash");
  self->client = client;
...
static voxgig_value* flash_run_op(flash_entity* self, Context* ctx,
                                    flash_postdone_fn post_done, PNError** err) {
...
voxgig_value* flash_stream(Entity* e, const char* action, voxgig_value* args,
                             voxgig_value* callopts, PNError** err) {
  flash_entity* self = (flash_entity*)e;
  *err = NULL;
  ...
  Context* ctx = make_context_util(cs, flash_ent_ctx(self));
...
static Entity* flash_make(Entity* e) {
  ...
  return flash_entity_new(self->client, opts);
}
...
static voxgig_value* flash_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(flash_ent_ctx(self), "SetData");
  }
  feature_hook_util(flash_ent_ctx(self), "GetData");
...
static voxgig_value* flash_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(flash_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(flash_ent_ctx(self), "GetMatch");
