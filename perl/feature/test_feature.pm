...
package ThesmsworksTestFeature;
...
sub init {
  ...
  my $entity = ThesmsworksHelpers::gp($options, 'entity');
  ...
  my $test_fetcher = sub {
    ...
    my $op = $fctx->{op};
    my $entmap = ThesmsworksHelpers::gp($entity, $op->{entity});
    ...
    my $resolve_match = sub {
      ...
      for my $src ($fctx->{match}, $fctx->{data}) {
        ...
        my $v = ThesmsworksHelpers::gp($src, 'id');
    ...
    if ('load' eq $op->{name}) {
      ...
      if (Voxgig::Struct::ismap($fctx->{reqdata})) {
        ...
        if ($op->{alias}) {
          my $alias_id = ThesmsworksHelpers::gp($op->{alias}, 'id');
      ...
      if (Voxgig::Struct::ismap($ent)) {
        my $id = ThesmsworksHelpers::gp($ent, 'id');
  ...
  my $net = ThesmsworksHelpers::gp($options, 'net');
...
sub make_netsim {
  ...
  my $do_sleep = sub {
    ...
    if (ref $net->{sleep} eq 'CODE') {
      ...
      ThesmsworksHelpers::sleep_ms($ms);
    }
  };
  ...
  return sub {
    ...
    if (ThesmsworksHelpers::is_true($net->{offline})) {
    ...
    if ($keys) {
      for my $key (@$keys) {
        ...
        if ($is_id || $is_reqd) {
          ...
          my $ka = $op->{alias} ? ThesmsworksHelpers::gp($op->{alias}, $key) : undef;
