...
sub new {
  ...
  my $gcp = \&ThesmsworksHelpers::get_ctx_prop;
  ...
  $self->{ctrl} = ThesmsworksControl->new();
  ...
  $self->{data} = ThesmsworksHelpers::to_map($gcp->($ctxmap, 'data')) || {};
  $self->{reqdata} = ThesmsworksHelpers::to_map($gcp->($ctxmap, 'reqdata')) || {};
  $self->{match} = ThesmsworksHelpers::to_map($gcp->($ctxmap, 'match')) || {};
  $self->{reqmatch} = ThesmsworksHelpers::to_map($gcp->($ctxmap, 'reqmatch')) || {};

  my $pt = $gcp->($ctxmap, 'point');
  $self->{point} = Voxgig::Struct::ismap($pt) ? $pt : ($basectx ? $basectx->{point} : undef);

  my $sp = $gcp->($ctxmap, 'spec');
  $self->{spec} = (Scalar::Util::blessed($sp) && $sp->isa('ThesmsworksSpec'))
    ? $sp : ($basectx ? $basectx->{spec} : undef);

  my $r = $gcp->($ctxmap, 'result');
  $self->{result} = (Scalar::Util::blessed($r) && $r->isa('ThesmsworksResult'))
    ? $r : ($basectx ? $basectx->{result} : undef);

  my $rp = $gcp->($ctxmap, 'response');
  $self->{response} = (Scalar::Util::blessed($rp) && $rp->isa('ThesmsworksResponse'))
...
sub resolve_op {
  ...
  return ThesmsworksOperation->new({}) if '' eq $opname;

  my $opcfg = ThesmsworksHelpers::gpath($self->{config}, "entity.$entname.op.$opname");

  my $input = ($opname eq 'update' || $opname eq 'create') ? 'data' : 'match';

  my $points = [];
  if (Voxgig::Struct::ismap($opcfg)) {
    my $t = ThesmsworksHelpers::gp($opcfg, 'points');
    $points = $t if Voxgig::Struct::islist($t);
  }

  my $op = ThesmsworksOperation->new({
    entity => $entname,
    name   => $opname,
    input  => $input,
    points => $points,
...
sub make_error {
  my ($self, $code, $msg) = @_;
  return ThesmsworksError->new($code, $msg, $self);
}
