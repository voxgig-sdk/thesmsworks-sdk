...
sub make {
  ...
  return BatchMessageEntity->new($self->{_client}, $opts);
}
...
# Returns the current BatchMessage data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of BatchMessage fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (BatchMessageCreateData shape). ctrl: optional
# per-call control. Returns the created BatchMessage data (hashref); dies
...
#
# reqmatch: match criteria hashref (id/query fields; BatchMessageRemoveMatch
# shape). ctrl: optional per-call control. Returns the removed BatchMessage
# data (hashref); dies with ThesmsworksError on failure.
sub remove {
