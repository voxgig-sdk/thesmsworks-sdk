# Thesmsworks SDK Swagger entity
...
sub make {
  ...
  return SwaggerEntity->new($self->{_client}, $opts);
}
...
# Returns the current Swagger data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Swagger fields).
sub match_get {
  my ($self) = @_;
