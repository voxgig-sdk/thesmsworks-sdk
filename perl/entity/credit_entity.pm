...
sub make {
  ...
  return CreditEntity->new($self->{_client}, $opts);
}
...
# Returns the current Credit data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Credit fields).
sub match_get {
  my ($self) = @_;
...
# Load a single Credit.
#
# reqmatch: match criteria hashref (id/query fields; CreditLoadMatch
...
# Returns the loaded Credit data (hashref); dies with ThesmsworksError
# on failure.
sub load {
