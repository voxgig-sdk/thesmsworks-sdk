...
sub make {
  ...
  return FlashEntity->new($self->{_client}, $opts);
}
...
# Returns the current Flash data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Flash fields).
sub match_get {
  my ($self) = @_;
