...
sub new {
  ...
  elsif (ThesmsworksHelpers::is_false($entopts->{active})) {
    # keep false
  }
  else {
    $entopts->{active} = Voxgig::Struct::JTRUE();
...
sub make {
  ...
  return OneTimePasswordEntity->new($self->{_client}, $opts);
}

sub data_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_data} = ThesmsworksHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetData');
  }
  return;
}

# Returns the current OneTimePassword data (hashref).
sub data_get {
  my ($self) = @_;
...
sub match_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_match} = ThesmsworksHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetMatch');
  }
  return;
}

# Returns the current match filter (any subset of OneTimePassword fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; OneTimePasswordLoadMatch
...
# Returns the loaded OneTimePassword data (hashref); dies with ThesmsworksError
# on failure.
sub load {
  ...
  return $self->_run_op($ctx, sub {
    ...
    if ($result) {
      ...
      if ($result->{resdata}) {
        $self->{_data} = ThesmsworksHelpers::to_map(
...
#
# reqdata: body data hashref (OneTimePasswordCreateData shape). ctrl: optional
# per-call control. Returns the created OneTimePassword data (hashref); dies
# with ThesmsworksError on failure.
sub create {
...
sub stream {
  ...
  my $utility = $self->{_utility};
  $callopts = ThesmsworksHelpers::to_map($callopts) || {};
  my $signal = ThesmsworksHelpers::gp($callopts, 'signal');
  my $ctrl = ThesmsworksHelpers::to_map(
    ThesmsworksHelpers::gp($callopts, 'ctrl')) || {};
  ...
  my $ctx = $utility->{make_context}->({
    ...
    'match' => $self->{_match},
    'data' => $self->{_data},
    %{ ThesmsworksHelpers::to_map($args) || {} },
  }, $self->{_entctx});

  # Outbound: expose an async-iterable/list payload so the request builder /
  # transport can stream it as the request body.
  my $body = ThesmsworksHelpers::gp($callopts, 'body');
  if (defined $body) {
    my $reqdata = ThesmsworksHelpers::to_map($ctx->{reqdata}) || {};
    $reqdata->{'body$'} = $body;
    $ctx->{reqdata} = $reqdata;
    $ctx->{stream_out} = $body;
  }
