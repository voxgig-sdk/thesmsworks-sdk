...
sub make {
  ...
  return UtilEntity->new($self->{_client}, $opts);
}
...
# Returns the current Util data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Util fields).
sub match_get {
  my ($self) = @_;
...
# Load a single Util.
#
# reqmatch: match criteria hashref (id/query fields; UtilLoadMatch
...
# Returns the loaded Util data (hashref); dies with ThesmsworksError
# on failure.
sub load {
