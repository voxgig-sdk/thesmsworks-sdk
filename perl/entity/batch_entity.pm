...
# Returns the current Batch data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Batch fields).
sub match_get {
  my ($self) = @_;
...
# Load a single Batch.
#
# reqmatch: match criteria hashref (id/query fields; BatchLoadMatch
...
# Returns the loaded Batch data (hashref); dies with ThesmsworksError
# on failure.
sub load {
