...
sub resp {
  ...
  return ThesmsworksResponse->new({
    'status' => $status,
...
sub is_sdk_error {
  my ($err) = @_;
  return Scalar::Util::blessed($err) && $err->isa('ThesmsworksError');
}
...

{
  ...
  $ctx->{spec} = spec_of();
  $ctx->{result} = ThesmsworksResult->new({ 'ok' => 1, 'resdata' => [] });
  my ($result, $err) = $utility->{make_result}->($ctx);
...
{
  my $ctx = make_ctx();
  my $preset = ThesmsworksResult->new({ 'ok' => 1 });
...
{
  my $boom = ThesmsworksError->new('boom', 'boom');
...

{
  ...
  $u->{make_fetch_def} = sub {
    return (undef, ThesmsworksError->new('fetchdef_boom', 'boom'));
  };
  my $ctx = make_ctx(utility => $u);
  $ctx->{spec} = request_spec();
  my ($response, $err) = $u->{make_request}->($ctx);
...
{
  my $ctx = make_ctx();
  $ctx->{result} = ThesmsworksResult->new({ 'ok' => 1, 'resdata' => 42 });
  is($utility->{done}->($ctx), 42, 'done returns resdata on success');
}

{
  my $ctx = make_ctx();
  $ctx->{result} = ThesmsworksResult->new({ 'ok' => 0 });
...

{
  ...
  $ctx->{result} = ThesmsworksResult->new({ 'ok' => 1, 'resdata' => 7 });
...

{
  ...
  $ctx->{result} = ThesmsworksResult->new({ 'ok' => 0, 'resdata' => 'fallback' });
...
{
  my $oclient = ThesmsworksSDK->test(undef, undef);
  ...
  $oclient->{features} = [];
  my $a = ThesmsworksBaseFeature->new;
  my $b = ThesmsworksBaseFeature->new;
...
sub named_feature {
  my ($name) = @_;
  my $f = ThesmsworksBaseFeature->new;
  $f->{name} = $name;
  return $f;
}
...

{
  ...
  ok(ThesmsworksHelpers::is_true($o2->{feature}{metrics}{active}),
    'feature order: array normalized to a map (metrics opts preserved)');
  ok(ThesmsworksHelpers::is_true($o2->{feature}{test}{active}),
...
sub auth_ctx {
  ...
  my $ctx = ThesmsworksContext->new({
    'client' => PipelineOptClient->new($options),
    'utility' => $utility,
    'opname' => 'load',
  }, undef);
  $ctx->{spec} = defined $headers
    ? ThesmsworksSpec->new({ 'headers' => $headers, 'step' => 's' })
    : undef;
  return $ctx;
}
...
{
  my $ctx = make_ctx();
  $ctx->{response} = ThesmsworksResponse->new({ 'status' => 200 });
  $ctx->{result} = ThesmsworksResult->new({});
...
{
  my $ctx = make_ctx();
  $ctx->{response} = ThesmsworksResponse->new({
