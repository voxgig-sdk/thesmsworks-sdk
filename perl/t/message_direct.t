...
DIRECT_LOAD: {
  my $setup = message_direct_setup({ 'id' => 'direct01' });
  my ($_should_skip, $_reason) = ThesmsworksTestRunner::is_control_skipped(
    'direct', 'direct-load-message', $setup->{live} ? 'live' : 'unit');
  if ($_should_skip) {
    note($_reason || 'skipped via sdk-test-control.json');
    pass('direct-load-message: skipped via sdk-test-control.json');
    last DIRECT_LOAD;
  }
  if ($setup->{live}) {
    note('live direct-load needs real ID - set *_ENTID env var with real IDs to run');
    pass('direct-load-message: skipped (live)');
    last DIRECT_LOAD;
  }
  ...
  if ($setup->{live}) {
    ...
    pass('direct-load-message: live ok');
  }
  else {
    ok(!defined $result->{err}, 'direct-load-message: no error');
    ok($result->{ok}, 'direct-load-message: ok');
    is(ThesmsworksHelpers::to_int($result->{status}), 200, 'direct-load-message: status');
    ok(defined $result->{data}, 'direct-load-message: data');
    if (Voxgig::Struct::ismap($result->{data})) {
      is($result->{data}{id}, 'direct01', 'direct-load-message: id');
    }
    is(scalar @{ $setup->{calls} }, 1, 'direct-load-message: 1 call');
  }
}


sub message_direct_setup {
  my ($mockres) = @_;
