...

{
  ...
  my $ent = $testsdk->Message(undef);
  ok(defined $ent, 'message: create instance');
}

BASIC_FLOW: {
  my $setup = message_basic_setup(undef);
  my $_live = $setup->{live} ? 1 : 0;
  # Per-op sdk-test-control.json skip.
  for my $_op ('create', 'load', 'remove') {
    my ($_should_skip, $_reason) = ThesmsworksTestRunner::is_control_skipped(
      'entityOp', "message." . $_op, $_live ? 'live' : 'unit');
    if ($_should_skip) {
      note($_reason || 'skipped via sdk-test-control.json');
      pass('message: basic flow skipped via sdk-test-control.json');
      last BASIC_FLOW;
    }
  }
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set THESMSWORKS_TEST_MESSAGE_ENTID JSON to run live');
    pass('message: basic flow skipped (synthetic IDs only)');
    last BASIC_FLOW;
  }
  ...
  # CREATE
  $V{message_ref01_ent} = $client->Message(undef);
  $V{message_ref01_data} = ThesmsworksHelpers::to_map(ThesmsworksHelpers::gp(
  ...
  $V{message_ref01_ent}->remove($V{message_ref01_match_rm0}, undef);
  pass('message remove: completed');

}

sub message_basic_setup {
  my ($extra) = @_;
  ThesmsworksTestRunner::load_env_local();

  my $entity_data_file = Cwd::abs_path(
    "$FindBin::Bin/../../.sdk/test/entity/message/MessageTestData.json");
  my $entity_data = do {
  ...
  # skip.
  my $entid_env_raw = $ENV{'THESMSWORKS_TEST_MESSAGE_ENTID'};
  ...
  my $idmap_resolved = ThesmsworksHelpers::to_map($env->{'THESMSWORKS_TEST_MESSAGE_ENTID'});
