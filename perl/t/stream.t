...
sub has_feature {
  my ($name) = @_;
  my $f = ThesmsworksConfig::make_config()->{feature};
...
sub find_list_entity {
  ...
  my $entities = ThesmsworksHelpers::to_map(
    ThesmsworksHelpers::gp($config, 'entity')) || {};
  for my $name (sort keys %$entities) {
    my $points = ThesmsworksHelpers::gpath($config, "entity.$name.op.list.points");
    next unless Voxgig::Struct::islist($points);
    for my $p (@$points) {
      my $params = ThesmsworksHelpers::gpath($p, 'args.params');
      my $reqd = Voxgig::Struct::islist($params)
        ? scalar(grep { ThesmsworksHelpers::is_true($_->{reqd}) } @$params)
        : 0;
...
SKIP: {
  ...
  {
    my $sdk = ThesmsworksSDK->test({ 'entity' => $seed }, undef);
