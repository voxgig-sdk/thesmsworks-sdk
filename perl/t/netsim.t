...
{
  my $sdk = ThesmsworksSDK->test({ 'net' => { 'offline' => 1 } }, undef);
...
{
  my $sdk = ThesmsworksSDK->test({ 'net' => { 'failTimes' => 1, 'failStatus' => 503 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $sdk = ThesmsworksSDK->test({ 'net' => { 'errorTimes' => 1 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $delay = 60;
  my $sdk = ThesmsworksSDK->test({ 'net' => { 'latency' => $delay } }, undef);
...
{
  my $sdk = ThesmsworksSDK->test(undef, undef);
