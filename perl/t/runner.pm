...
sub env_override {
  my ($m) = @_;
  my $live = getenv('THESMSWORKS_TEST_LIVE');
  my $override = getenv('THESMSWORKS_TEST_OVERRIDE');
  ...
  my $explain = getenv('THESMSWORKS_TEST_EXPLAIN');
  $m->{'THESMSWORKS_TEST_EXPLAIN'} = $explain if defined $explain && '' ne $explain;
...
sub is_control_skipped {
  ...
  my $skip = ThesmsworksHelpers::gpath($ctrl, "test.skip.$mode") || {};
  my $items = $skip->{$kind} || [];
...
sub live_client_options {
  my $ctrl = load_test_control();
  my $opts = ThesmsworksHelpers::gpath($ctrl, 'test.client.options');
...
sub live_delay_ms {
  my $ctrl = load_test_control();
  my $v = ThesmsworksHelpers::gpath($ctrl, 'test.live.delayMs');
