...
#    `contextify` answers a TIED plain hash (ThesmsworksOmniViewTie) whose
...
package ThesmsworksOmniEntityRef;
...
# --------------------------------------------------------------------------
package ThesmsworksOmniViewTie;
...
sub _iserr { return Scalar::Util::blessed($_[0]) && $_[0]->isa('ThesmsworksError') }
...
sub _wrap {
  ...
  if ('ok' eq $name && Scalar::Util::blessed($t) && $t->isa('ThesmsworksResult')
    && defined $val && !ref $val) {
  ...
  if (ref($val) eq 'HASH' && tied(%$val)
    && tied(%$val)->isa('ThesmsworksOmniViewTie')) {
    my $inner = tied(%$val)->{target};
    return $inner if $seen->{ Scalar::Util::refaddr($inner) };
    return ThesmsworksOmni::objview($inner,
  ...
  if (Scalar::Util::blessed($val)) {
    ...
    if (('HASH' eq (Scalar::Util::reftype($val) || ''))) {
      ...
      return ThesmsworksOmni::objview($val,
        { %$seen, Scalar::Util::refaddr($t) => 1 });
    }
  ...
  if (ref($val) eq 'HASH' || ref($val) eq 'ARRAY') {
    return ThesmsworksOmni::toomni($val);
  }
...
# --------------------------------------------------------------------------
package ThesmsworksOmniProviderTie;
...
package ThesmsworksOmniProvider;
...
sub objview {
  my ($target, $seen) = @_;
  tie my %view, 'ThesmsworksOmniViewTie', $target, $seen;
  return \%view;
}
...
sub isview {
  ...
  return ref($val) eq 'HASH' && tied(%$val)
    && tied(%$val)->isa('ThesmsworksOmniViewTie') ? 1 : 0;
}
...
sub sdkhooks {
  ...
  my $struct = $utility->{struct} || ThesmsworksHelpers::struct_facade();
  ...
  my $subject = sub {
    ...
    $found = $utility->{ ThesmsworksOmniViewTie::_snake($name) } if !defined $found;
  ...
  my $contextify = sub {
    ...
    if (Voxgig::Struct::ismap($ent) && !Scalar::Util::blessed($ent)
      ...
      $ctxmap->{entity} = ThesmsworksOmniEntityRef->new($ent->{name});
    }
    my $ctx = ThesmsworksContext->new($ctxmap, undef);
    enrich($ctxmap, $ctx);
    $ctx->{utility} = $utility;
...
sub sdkprovider {
  my ($sdk) = @_;
  tie my %provider, 'ThesmsworksOmniProviderTie', $sdk, sdkhooks($sdk);
  return bless \%provider, 'ThesmsworksOmniProvider';
}
