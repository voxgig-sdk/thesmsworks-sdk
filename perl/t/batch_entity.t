...

{
  ...
  my $ent = $testsdk->Batch(undef);
  ok(defined $ent, 'batch: create instance');
}

BASIC_FLOW: {
  my $setup = batch_basic_setup(undef);
  my $_live = $setup->{live} ? 1 : 0;
  # Per-op sdk-test-control.json skip.
  for my $_op ('load') {
    my ($_should_skip, $_reason) = ThesmsworksTestRunner::is_control_skipped(
      'entityOp', "batch." . $_op, $_live ? 'live' : 'unit');
    if ($_should_skip) {
      note($_reason || 'skipped via sdk-test-control.json');
      pass('batch: basic flow skipped via sdk-test-control.json');
      last BASIC_FLOW;
    }
  }
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set THESMSWORKS_TEST_BATCH_ENTID JSON to run live');
    pass('batch: basic flow skipped (synthetic IDs only)');
    last BASIC_FLOW;
  }
  ...
  $V{batch_ref01_data_raw} = Voxgig::Struct::items(ThesmsworksHelpers::to_map(
    ThesmsworksHelpers::gpath($setup->{data}, 'existing.batch')));
  $V{batch_ref01_data} = undef;
  if (@{ $V{batch_ref01_data_raw} || [] }) {
    $V{batch_ref01_data} = ThesmsworksHelpers::to_map($V{batch_ref01_data_raw}[0][1]);
  }
...
sub batch_basic_setup {
  my ($extra) = @_;
  ThesmsworksTestRunner::load_env_local();

  my $entity_data_file = Cwd::abs_path(
    "$FindBin::Bin/../../.sdk/test/entity/batch/BatchTestData.json");
  my $entity_data = do {
  ...
  # skip.
  my $entid_env_raw = $ENV{'THESMSWORKS_TEST_BATCH_ENTID'};
  ...
  my $idmap_resolved = ThesmsworksHelpers::to_map($env->{'THESMSWORKS_TEST_BATCH_ENTID'});
