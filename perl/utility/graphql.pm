# Thesmsworks SDK utility: graphql
#
...
sub graphql_error_code {
  ...
  my $ext = ThesmsworksHelpers::gp($gqlerr, 'extensions');

  my $raw = ThesmsworksHelpers::gp($ext, 'code');
  $raw = ThesmsworksHelpers::gp($ext, 'type') if !defined $raw || '' eq $raw;
...
$REGISTRY{graphql_body} = sub {
  ...
  my $gql = ThesmsworksHelpers::gp($ctx->{point}, 'graphql');
  ...
  my $varlist = ThesmsworksHelpers::gp($gql, 'vars');
  $varlist = [] unless ref($varlist) eq 'ARRAY';

  for my $spec (@$varlist) {
    next unless ref($spec) eq 'HASH';

    my $name = ThesmsworksHelpers::gp($spec, 'name');
    my $from = ThesmsworksHelpers::gp($spec, 'from');
    ...
    my $val = ThesmsworksHelpers::gp($reqsrc, $from);
    $val = ThesmsworksHelpers::gp($datasrc, $from) unless defined $val;
    $variables{$name} = $val if defined $val;
  }

  return {
    query     => ThesmsworksHelpers::gp($gql, 'doc'),
    variables => \%variables,
  };
};
...
$REGISTRY{graphql_errors} = sub {
  ...
  my $kind = ThesmsworksHelpers::gp($ctx->{point}, 'kind');
  return 0 unless defined $kind && 'graphql' eq $kind;

  my $errors = ThesmsworksHelpers::gp($result->{body}, 'errors');
  return 0 unless ref($errors) eq 'ARRAY' && 0 < scalar(@$errors);

  my $first = $errors->[0];
  my $msg = ThesmsworksHelpers::gp($first, 'message');
