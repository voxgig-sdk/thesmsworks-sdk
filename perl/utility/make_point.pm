# Thesmsworks SDK utility: make_point
...
sub _parts_len {
  my ($point) = @_;
  my $parts = ThesmsworksHelpers::gp($point, 'parts');
...
$REGISTRY{make_point} = sub {
  ...
  if ($ctx->{out}{point}) {
    ...
    return (undef, $preset)
      if Scalar::Util::blessed($preset) && $preset->isa('ThesmsworksError');
    $ctx->{point} = $preset;
  ...
  my $allow_op = ThesmsworksHelpers::gpath($options, 'allow.op');
  ...
  if (1 == @{ $op->{points} }) {
    ...
    my $matched = 0;
    for my $p (@{ $op->{points} }) {
      my $select_def = ThesmsworksHelpers::to_map(ThesmsworksHelpers::gp($p, 'select'));
      my $found = 1;

      if ($selector && $select_def) {
        my $exist = ThesmsworksHelpers::gp($select_def, 'exist');
        if (Voxgig::Struct::islist($exist)) {
          for my $ek (@$exist) {
            my $rv = ThesmsworksHelpers::gp($reqselector, "$ek");
            my $sv = ThesmsworksHelpers::gp($selector, "$ek");
      ...
      if ($found) {
        my $req_action = ThesmsworksHelpers::gp($reqselector, '$action');
        my $select_action = ThesmsworksHelpers::gp($select_def, '$action');
        $found = 0 unless ThesmsworksHelpers::eqv($req_action, $select_action);
      }
    ...
    unless ($matched) {
      ...
      my $unmatched_action = $reqselector
        ? ThesmsworksHelpers::gp($reqselector, '$action') : undef;
    ...
    if ($reqselector) {
      ...
      if (defined $req_action && $point) {
        my $point_select = ThesmsworksHelpers::to_map(ThesmsworksHelpers::gp($point, 'select'));
        my $point_action = ThesmsworksHelpers::gp($point_select, '$action');
        unless (ThesmsworksHelpers::eqv($req_action, $point_action)) {
          return (undef, $ctx->make_error('point_action_invalid',
            "Operation \"$op->{name}\" action \""
