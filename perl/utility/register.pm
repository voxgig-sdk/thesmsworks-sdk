...
$ThesmsworksUtility::REGISTRAR = sub {
  my ($u) = @_;
  for my $k (keys %ThesmsworksUtilities::REGISTRY) {
    $u->{$k} = $ThesmsworksUtilities::REGISTRY{$k};
  }
  # The vendored struct utility, reachable as $utility->{struct}{<fn>}.
  $u->{struct} = ThesmsworksHelpers::struct_facade();
};
