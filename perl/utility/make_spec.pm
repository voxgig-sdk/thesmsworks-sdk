...
$REGISTRY{make_spec} = sub {
  ...
  my $base = ThesmsworksHelpers::gp($options, 'base');
  $base = '' unless defined $base;
  my $prefix = ThesmsworksHelpers::gp($options, 'prefix');
  $prefix = '' unless defined $prefix;
  my $suffix = ThesmsworksHelpers::gp($options, 'suffix');
  $suffix = '' unless defined $suffix;

  my $parts = [];
  $parts = ThesmsworksHelpers::gp($point, 'parts') if $point;
  ...
  my $allow_method = ThesmsworksHelpers::gpath($options, 'allow.method');
  ...
  my $pkind = ThesmsworksHelpers::gp($ctx->{point}, 'kind');
  if (defined $pkind && 'graphql' eq $pkind) {
    ...
    $ctx->{spec}{headers}{'content-type'} =
      $ThesmsworksUtilities::GRAPHQL_CONTENT_TYPE;
  }
  else {
