# Thesmsworks SDK utility: make_options
...
$REGISTRY{make_options} = sub {
  ...
  ThesmsworksHelpers::coerce_bools($opts);
  ...
  my $sys_fetch = ThesmsworksHelpers::gpath($opts, 'system.fetch');

  # CLONE the config side: `config` is a process-wide singleton
  # (ThesmsworksConfig::shared_config) and merge uses its nested hashes as merge
  ...
  my $clean_keys = ThesmsworksHelpers::gpath($opts, 'clean.keys');
  ...
  if (!@featureorder) {
    my $fmap = ThesmsworksHelpers::to_map(
      ThesmsworksHelpers::gp($opts, 'feature')) || {};
