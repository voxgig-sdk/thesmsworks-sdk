// Generated direct-call tests for the message entity (mirrors the java
...
object MessageDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-load-message") {
      ...
      rep.eq("direct-load-message.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-message.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-message.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-message.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-message.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-load-message.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
    }
  }
}
