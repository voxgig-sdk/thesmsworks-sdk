...
import voxgig.thesmsworkssdk.core.*
...
object PrimaryCorpus {
  ...
  def toValue(o: Object): Value = o match {
    ...
    case x if x eq voxgig.thesmsworkssdk.utility.struct.Struct.UNDEF => VNull
    case m: JMap[_, _] =>
  ...
  private def corpusCtx(client: ThesmsworksSDK, ctxmap: Value): Context = {
  ...
  private def runset(name: String, client: ThesmsworksSDK)(f: (Context, Seq[Value]) => Value): Unit = {
  ...
  private def clientFor(name: String, shared: ThesmsworksSDK): ThesmsworksSDK =
    sectionSetup(sectionRun(name)) match {
      case m @ VMap(_) => ThesmsworksSDK.testSDK(null, jmap(m))
      case _           => shared
    }
  ...
  def run(testfile: String): Int = {
    ...
    val sdk = ThesmsworksSDK.testSDK()
