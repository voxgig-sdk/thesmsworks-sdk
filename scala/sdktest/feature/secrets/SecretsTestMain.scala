...
import voxgig.thesmsworkssdk.core._
import voxgig.thesmsworkssdk.feature._
...
object SecretsTestMain {
  ...
  private val ENVPREFIX = "THESMSWORKS_TEST_SECRETS_"
  ...
  private def secretsFeatureOf(client: ThesmsworksSDK): SecretsFeature = {
  ...
  private def withSecrets(build: Boolean => ThesmsworksSDK): ThesmsworksSDK = {
    val client = build(false)
    if (null == secretsFeatureOf(client)) build(true) else client
  }

  private def secretsClient(w: Wire, extra: (String, Object)*): ThesmsworksSDK =
    withSecrets { extend =>
      val opts = sdkopts(w, extra*)
      if (extend) opts.put("extend", jlist(new SecretsFeature()))
      new ThesmsworksSDK(opts)
    }

  private def testModeClient(w: Wire, extra: (String, Object)*): ThesmsworksSDK =
    withSecrets { extend =>
      val opts = sdkopts(w, extra*)
      if (extend) opts.put("extend", jlist(new SecretsFeature()))
      ThesmsworksSDK.testSDK(null, opts)
    }
  ...
  private def entities(client: ThesmsworksSDK): List[SdkEntity] = {
  ...
  private def driveUntil(client: ThesmsworksSDK, stop: () => Boolean): (Boolean, String) = {
    var lasterr = ""
    var done = false
  ...
  private def driveOneCall(client: ThesmsworksSDK, w: Wire): Boolean = {
  ...
  private def testInactive(): Unit = {
    val client = ThesmsworksSDK.testSDK(null, jmap("apikey" -> "OPTKEY01"))
    ...
    val bare = ThesmsworksSDK.testSDK(null, null)
