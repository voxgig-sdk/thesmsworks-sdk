// Generated direct-call tests for the batch entity (mirrors the java
...
object BatchDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-load-batch") {
      ...
      rep.eq("direct-load-batch.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-batch.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-batch.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-batch.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-batch.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-load-batch.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
    }
  }
}
