package voxgig.thesmsworkssdk.core

import java.util.{Map => JMap}

import voxgig.thesmsworkssdk.utility.Json

// Static SDK configuration and by-name feature construction.
object Config {

  def makeConfig(): JMap[String, Object] =
    Json.parse(configJson()).asInstanceOf[JMap[String, Object]]

  // SHARED CONFIG (sdkgen rung L2).
  //
  // The SDK reads the config on every request and never writes to it, so one
  // instance is shared by every client rather than rebuilt per client - the
  // difference between parsing the embedded JSON once and once per client.
  //
  // A lazy val is initialised once, on first use, under a lock, so
  // concurrent first calls build it exactly once.
  //
  // The returned map is SHARED: treat it as read-only. Callers that need to
  // mutate should use makeConfig, which always parses a fresh copy.
  private lazy val sharedConfigVal: JMap[String, Object] = makeConfig()

  def sharedConfig(): JMap[String, Object] = sharedConfigVal

  // The plugin definitions the model selected per feature, as List[Any] so a
  // feature consumes them without core naming a vendored type. Empty when no
  // active feature declares active plugin groups for this target.
  def featurePlugins(name: String): List[Any] = name match {
    case _ => Nil
  }

  def makeFeature(name: String): Feature = name match {
    case "debug" => new voxgig.thesmsworkssdk.feature.DebugFeature()
    case "idempotency" => new voxgig.thesmsworkssdk.feature.IdempotencyFeature()
    case "metrics" => new voxgig.thesmsworkssdk.feature.MetricsFeature()
    case "paging" => new voxgig.thesmsworkssdk.feature.PagingFeature()
    case "ratelimit" => new voxgig.thesmsworkssdk.feature.RatelimitFeature()
    case "retry" => new voxgig.thesmsworkssdk.feature.RetryFeature()
    case "test" => new voxgig.thesmsworkssdk.feature.TestFeature()
    case "timeout" => new voxgig.thesmsworkssdk.feature.TimeoutFeature()
    case _ => new voxgig.thesmsworkssdk.feature.BaseFeature()
  }

  private def configJson(): String = {
    val b = new StringBuilder()
    b.append("{")
    b.append(" \"main\": {")
    b.append("  \"name\": \"Thesmsworks\"")
    b.append(" },")
    b.append(" \"feature\": {")
    b.append("  \"debug\": {")
    b.append("   \"options\": {")
    b.append("    \"active\": false,")
    b.append("    \"max\": 100,")
    b.append("    \"redact\": [")
    b.append("     \"authorization\",")
    b.append("     \"cookie\",")
    b.append("     \"set-cookie\",")
    b.append("     \"api-key\",")
    b.append("     \"apikey\",")
    b.append("     \"x-api-key\",")
    b.append("     \"idempotency-key\"")
    b.append("    ]")
    b.append("   },")
    b.append("   \"optspec\": {")
    b.append("    \"now\": \"`$FUNCTION`\",")
    b.append("    \"onEntry\": \"`$FUNCTION`\"")
    b.append("   },")
    b.append("   \"strict\": false")
    b.append("  },")
    b.append("  \"idempotency\": {")
    b.append("   \"options\": {")
    b.append("    \"active\": false,")
    b.append("    \"header\": \"Idempotency-Key\",")
    b.append("    \"methods\": [")
    b.append("     \"POST\",")
    b.append("     \"PUT\",")
    b.append("     \"PATCH\",")
    b.append("     \"DELETE\"")
    b.append("    ],")
    b.append("    \"ops\": [")
    b.append("     \"create\",")
    b.append("     \"update\",")
    b.append("     \"remove\"")
    b.append("    ]")
    b.append("   },")
    b.append("   \"optspec\": {")
    b.append("    \"keygen\": \"`$FUNCTION`\"")
    b.append("   },")
    b.append("   \"strict\": false")
    b.append("  },")
    b.append("  \"metrics\": {")
    b.append("   \"options\": {")
    b.append("    \"active\": false")
    b.append("   },")
    b.append("   \"optspec\": {")
    b.append("    \"now\": \"`$FUNCTION`\"")
    b.append("   },")
    b.append("   \"strict\": false")
    b.append("  },")
    b.append("  \"paging\": {")
    b.append("   \"options\": {")
    b.append("    \"active\": false,")
    b.append("    \"afterVar\": \"after\",")
    b.append("    \"cursorParam\": \"cursor\",")
    b.append("    \"firstVar\": \"first\",")
    b.append("    \"limitParam\": \"limit\",")
    b.append("    \"pageParam\": \"page\",")
    b.append("    \"startPage\": 1")
    b.append("   },")
    b.append("   \"optspec\": {")
    b.append("    \"limit\": \"`$NUMBER`\",")
    b.append("    \"ops\": \"`$LIST`\"")
    b.append("   },")
    b.append("   \"strict\": false")
    b.append("  },")
    b.append("  \"ratelimit\": {")
    b.append("   \"options\": {")
    b.append("    \"active\": false,")
    b.append("    \"burst\": 5,")
    b.append("    \"rate\": 5")
    b.append("   },")
    b.append("   \"optspec\": {")
    b.append("    \"now\": \"`$FUNCTION`\",")
    b.append("    \"sleep\": \"`$FUNCTION`\"")
    b.append("   },")
    b.append("   \"strict\": false")
    b.append("  },")
    b.append("  \"retry\": {")
    b.append("   \"options\": {")
    b.append("    \"active\": false,")
    b.append("    \"factor\": 2,")
    b.append("    \"maxDelay\": 2000,")
    b.append("    \"minDelay\": 50,")
    b.append("    \"retries\": 2,")
    b.append("    \"statuses\": [")
    b.append("     408,")
    b.append("     425,")
    b.append("     429,")
    b.append("     500,")
    b.append("     502,")
    b.append("     503,")
    b.append("     504")
    b.append("    ]")
    b.append("   },")
    b.append("   \"optspec\": {")
    b.append("    \"jitter\": \"`$BOOLEAN`\",")
    b.append("    \"sleep\": \"`$FUNCTION`\"")
    b.append("   },")
    b.append("   \"strict\": false")
    b.append("  },")
    b.append("  \"test\": {")
    b.append("   \"options\": {")
    b.append("    \"active\": false")
    b.append("   },")
    b.append("   \"optspec\": {")
    b.append("    \"entity\": \"`$MAP`\",")
    b.append("    \"net\": \"`$MAP`\"")
    b.append("   },")
    b.append("   \"strict\": false")
    b.append("  },")
    b.append("  \"timeout\": {")
    b.append("   \"options\": {")
    b.append("    \"active\": false,")
    b.append("    \"ms\": 30000")
    b.append("   },")
    b.append("   \"optspec\": {")
    b.append("    \"clearTimer\": \"`$FUNCTION`\",")
    b.append("    \"setTimer\": \"`$FUNCTION`\"")
    b.append("   },")
    b.append("   \"strict\": false")
    b.append("  }")
    b.append(" },")
    b.append(" \"options\": {")
    b.append("  \"base\": \"https://api.thesmsworks.co.uk/v1\",")
    b.append("  \"auth\": {")
    b.append("   \"prefix\": \"\"")
    b.append("  },")
    b.append("  \"headers\": {")
    b.append("   \"content-type\": \"application/json\"")
    b.append("  },")
    b.append("  \"entity\": {")
    b.append("   \"batch\": {},")
    b.append("   \"batch_message\": {},")
    b.append("   \"credit\": {},")
    b.append("   \"flash\": {},")
    b.append("   \"message\": {},")
    b.append("   \"one_time_password\": {},")
    b.append("   \"schedule\": {},")
    b.append("   \"swagger\": {},")
    b.append("   \"util\": {}")
    b.append("  }")
    b.append(" },")
    b.append(" \"entity\": {")
    b.append("  \"batch\": {")
    b.append("   \"fields\": [")
    b.append("    {")
    b.append("     \"name\": \"id\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    }")
    b.append("   ],")
    b.append("   \"id\": {")
    b.append("    \"field\": \"id\",")
    b.append("    \"name\": \"id\"")
    b.append("   },")
    b.append("   \"name\": \"batch\",")
    b.append("   \"op\": {")
    b.append("    \"load\": {")
    b.append("     \"input\": \"data\",")
    b.append("     \"name\": \"load\",")
    b.append("     \"points\": [")
    b.append("      {")
    b.append("       \"args\": {")
    b.append("        \"params\": [")
    b.append("         {")
    b.append("          \"kind\": \"param\",")
    b.append("          \"name\": \"id\",")
    b.append("          \"orig\": \"batchid\",")
    b.append("          \"reqd\": true,")
    b.append("          \"type\": \"`$STRING`\"")
    b.append("         }")
    b.append("        ]")
    b.append("       },")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"GET\",")
    b.append("       \"orig\": \"/batch/{batchid}\",")
    b.append("       \"rename\": {")
    b.append("        \"param\": {")
    b.append("         \"batchid\": \"id\"")
    b.append("        }")
    b.append("       },")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"batch\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"var\": \"id\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"exist\": [")
    b.append("         \"id\"")
    b.append("        ]")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"batch\",")
    b.append("        \"{id}\"")
    b.append("       ]")
    b.append("      }")
    b.append("     ]")
    b.append("    }")
    b.append("   },")
    b.append("   \"relations\": {")
    b.append("    \"ancestors\": []")
    b.append("   }")
    b.append("  },")
    b.append("  \"batch_message\": {")
    b.append("   \"fields\": [")
    b.append("    {")
    b.append("     \"name\": \"ai\",")
    b.append("     \"short\": \"Used to determine whether The SMS Works AI Optimiser should be used in the event that the message is just longer than the 1 or 2 credit boundary.\",")
    b.append("     \"type\": \"`$BOOLEAN`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"content\",")
    b.append("     \"req\": true,")
    b.append("     \"short\": \"Message to send to the recipient\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"deliveryreporturl\",")
    b.append("     \"short\": \"The url to which we should POST delivery reports to for this message.\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"destinations\",")
    b.append("     \"req\": true,")
    b.append("     \"short\": \"Telephone numbers of each of the recipients\",")
    b.append("     \"type\": \"`$ARRAY`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"schedule\",")
    b.append("     \"short\": \"Date-time at which to send the batch.\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"sender\",")
    b.append("     \"req\": true,")
    b.append("     \"short\": \"The sender of the message.\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"tag\",")
    b.append("     \"short\": \"An identifying label for the message, which you can use to filter and report on messages you've sent later.\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"ttl\",")
    b.append("     \"short\": \"The number of minutes before the delivery report is deleted.\",")
    b.append("     \"type\": \"`$NUMBER`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"validity\",")
    b.append("     \"short\": \"The optional number of minutes to attempt delivery before the message is marked as EXPIRED.\",")
    b.append("     \"type\": \"`$NUMBER`\"")
    b.append("    }")
    b.append("   ],")
    b.append("   \"name\": \"batch_message\",")
    b.append("   \"op\": {")
    b.append("    \"create\": {")
    b.append("     \"input\": \"data\",")
    b.append("     \"name\": \"create\",")
    b.append("     \"points\": [")
    b.append("      {")
    b.append("       \"args\": {},")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"POST\",")
    b.append("       \"orig\": \"/batch/any\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"batch\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"any\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {},")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"batch\",")
    b.append("        \"any\"")
    b.append("       ]")
    b.append("      },")
    b.append("      {")
    b.append("       \"args\": {},")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"POST\",")
    b.append("       \"orig\": \"/batch/schedule\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"batch\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"schedule\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {},")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"batch\",")
    b.append("        \"schedule\"")
    b.append("       ]")
    b.append("      },")
    b.append("      {")
    b.append("       \"args\": {},")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"POST\",")
    b.append("       \"orig\": \"/batch/send\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"batch\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"send\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {},")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"batch\",")
    b.append("        \"send\"")
    b.append("       ]")
    b.append("      }")
    b.append("     ]")
    b.append("    },")
    b.append("    \"remove\": {")
    b.append("     \"input\": \"data\",")
    b.append("     \"name\": \"remove\",")
    b.append("     \"points\": [")
    b.append("      {")
    b.append("       \"args\": {")
    b.append("        \"params\": [")
    b.append("         {")
    b.append("          \"kind\": \"param\",")
    b.append("          \"name\": \"batchid\",")
    b.append("          \"orig\": \"batchid\",")
    b.append("          \"reqd\": true,")
    b.append("          \"type\": \"`$STRING`\"")
    b.append("         }")
    b.append("        ]")
    b.append("       },")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"DELETE\",")
    b.append("       \"orig\": \"/batches/schedule/{batchid}\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"batches\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"schedule\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"var\": \"batchid\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"exist\": [")
    b.append("         \"batchid\"")
    b.append("        ]")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"batches\",")
    b.append("        \"schedule\",")
    b.append("        \"{batchid}\"")
    b.append("       ]")
    b.append("      }")
    b.append("     ]")
    b.append("    }")
    b.append("   },")
    b.append("   \"relations\": {")
    b.append("    \"ancestors\": [")
    b.append("     [")
    b.append("      \"schedule\"")
    b.append("     ]")
    b.append("    ]")
    b.append("   }")
    b.append("  },")
    b.append("  \"credit\": {")
    b.append("   \"fields\": [],")
    b.append("   \"name\": \"credit\",")
    b.append("   \"op\": {")
    b.append("    \"load\": {")
    b.append("     \"input\": \"data\",")
    b.append("     \"name\": \"load\",")
    b.append("     \"points\": [")
    b.append("      {")
    b.append("       \"args\": {},")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"GET\",")
    b.append("       \"orig\": \"/credits/balance\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"credits\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"balance\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"$action\": \"balance\"")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"credits\",")
    b.append("        \"balance\"")
    b.append("       ]")
    b.append("      }")
    b.append("     ]")
    b.append("    }")
    b.append("   },")
    b.append("   \"relations\": {")
    b.append("    \"ancestors\": []")
    b.append("   }")
    b.append("  },")
    b.append("  \"flash\": {")
    b.append("   \"fields\": [],")
    b.append("   \"name\": \"flash\",")
    b.append("   \"op\": {},")
    b.append("   \"relations\": {")
    b.append("    \"ancestors\": []")
    b.append("   }")
    b.append("  },")
    b.append("  \"message\": {")
    b.append("   \"fields\": [")
    b.append("    {")
    b.append("     \"name\": \"credits\",")
    b.append("     \"short\": \"The number of credits used on the message.\",")
    b.append("     \"type\": \"`$NUMBER`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"destination\",")
    b.append("     \"short\": \"The phone number of the recipient.\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"from\",")
    b.append("     \"short\": \"The date-time from which you would like matching messages\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"id\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"keyword\",")
    b.append("     \"short\": \"The keyword used in the inbound message\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"limit\",")
    b.append("     \"short\": \"The maximum number of messages that you would like returned in this call.\",")
    b.append("     \"type\": \"`$NUMBER`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"metadata\",")
    b.append("     \"short\": \"An array of objects containing metadata key/value pairs that have been saved on messages.\",")
    b.append("     \"type\": \"`$OBJECT`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"sender\",")
    b.append("     \"short\": \"The sender of the message (this can be the configured sender name for an outbound message or the senders phone number for an inbound message).\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"skip\",")
    b.append("     \"short\": \"The number of results you would like to ignore before returning messages.\",")
    b.append("     \"type\": \"`$NUMBER`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"status\",")
    b.append("     \"short\": \"The status of the messages you would like returned (either 'SENT', 'DELIVERED', 'EXPIRED', 'UNDELIVERABLE', 'REJECTED' or 'INCOMING')\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"to\",")
    b.append("     \"short\": \"The date-time to which you would like matching messages\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"unread\",")
    b.append("     \"short\": \"In queries for incoming messages ('status' is 'INCOMING'), specify whether you explicitly want unread messages (true) or read messages (false).\",")
    b.append("     \"type\": \"`$BOOLEAN`\"")
    b.append("    }")
    b.append("   ],")
    b.append("   \"id\": {")
    b.append("    \"field\": \"id\",")
    b.append("    \"name\": \"id\"")
    b.append("   },")
    b.append("   \"name\": \"message\",")
    b.append("   \"op\": {")
    b.append("    \"create\": {")
    b.append("     \"input\": \"data\",")
    b.append("     \"name\": \"create\",")
    b.append("     \"points\": [")
    b.append("      {")
    b.append("       \"args\": {},")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"POST\",")
    b.append("       \"orig\": \"/message/flash\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"message\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"flash\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"$action\": \"flash\"")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"message\",")
    b.append("        \"flash\"")
    b.append("       ]")
    b.append("      },")
    b.append("      {")
    b.append("       \"args\": {},")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"POST\",")
    b.append("       \"orig\": \"/message/schedule\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"message\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"schedule\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"$action\": \"schedule\"")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"message\",")
    b.append("        \"schedule\"")
    b.append("       ]")
    b.append("      },")
    b.append("      {")
    b.append("       \"args\": {},")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"POST\",")
    b.append("       \"orig\": \"/message/send\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"message\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"send\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"$action\": \"send\"")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"message\",")
    b.append("        \"send\"")
    b.append("       ]")
    b.append("      },")
    b.append("      {")
    b.append("       \"args\": {},")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"POST\",")
    b.append("       \"orig\": \"/messages\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"messages\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {},")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"messages\"")
    b.append("       ]")
    b.append("      },")
    b.append("      {")
    b.append("       \"args\": {},")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"POST\",")
    b.append("       \"orig\": \"/messages/failed\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"messages\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"failed\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"$action\": \"failed\"")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"messages\",")
    b.append("        \"failed\"")
    b.append("       ]")
    b.append("      },")
    b.append("      {")
    b.append("       \"args\": {},")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"POST\",")
    b.append("       \"orig\": \"/messages/inbox\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"messages\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"inbox\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"$action\": \"inbox\"")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"messages\",")
    b.append("        \"inbox\"")
    b.append("       ]")
    b.append("      }")
    b.append("     ]")
    b.append("    },")
    b.append("    \"load\": {")
    b.append("     \"input\": \"data\",")
    b.append("     \"name\": \"load\",")
    b.append("     \"points\": [")
    b.append("      {")
    b.append("       \"args\": {")
    b.append("        \"params\": [")
    b.append("         {")
    b.append("          \"kind\": \"param\",")
    b.append("          \"name\": \"id\",")
    b.append("          \"orig\": \"messageid\",")
    b.append("          \"reqd\": true,")
    b.append("          \"type\": \"`$STRING`\"")
    b.append("         }")
    b.append("        ]")
    b.append("       },")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"GET\",")
    b.append("       \"orig\": \"/messages/{messageid}\",")
    b.append("       \"rename\": {")
    b.append("        \"param\": {")
    b.append("         \"messageid\": \"id\"")
    b.append("        }")
    b.append("       },")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"messages\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"var\": \"id\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"exist\": [")
    b.append("         \"id\"")
    b.append("        ]")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"messages\",")
    b.append("        \"{id}\"")
    b.append("       ]")
    b.append("      },")
    b.append("      {")
    b.append("       \"args\": {},")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"GET\",")
    b.append("       \"orig\": \"/messages/schedule\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"messages\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"schedule\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"$action\": \"schedule\"")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"messages\",")
    b.append("        \"schedule\"")
    b.append("       ]")
    b.append("      }")
    b.append("     ]")
    b.append("    },")
    b.append("    \"remove\": {")
    b.append("     \"input\": \"data\",")
    b.append("     \"name\": \"remove\",")
    b.append("     \"points\": [")
    b.append("      {")
    b.append("       \"args\": {")
    b.append("        \"params\": [")
    b.append("         {")
    b.append("          \"kind\": \"param\",")
    b.append("          \"name\": \"id\",")
    b.append("          \"orig\": \"messageid\",")
    b.append("          \"reqd\": true,")
    b.append("          \"type\": \"`$STRING`\"")
    b.append("         }")
    b.append("        ]")
    b.append("       },")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"DELETE\",")
    b.append("       \"orig\": \"/messages/{messageid}\",")
    b.append("       \"rename\": {")
    b.append("        \"param\": {")
    b.append("         \"messageid\": \"id\"")
    b.append("        }")
    b.append("       },")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"messages\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"var\": \"id\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"exist\": [")
    b.append("         \"id\"")
    b.append("        ]")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"messages\",")
    b.append("        \"{id}\"")
    b.append("       ]")
    b.append("      },")
    b.append("      {")
    b.append("       \"args\": {")
    b.append("        \"params\": [")
    b.append("         {")
    b.append("          \"kind\": \"param\",")
    b.append("          \"name\": \"messageid\",")
    b.append("          \"orig\": \"messageid\",")
    b.append("          \"reqd\": true,")
    b.append("          \"type\": \"`$STRING`\"")
    b.append("         }")
    b.append("        ]")
    b.append("       },")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"DELETE\",")
    b.append("       \"orig\": \"/messages/schedule/{messageid}\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"messages\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"schedule\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"var\": \"messageid\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"exist\": [")
    b.append("         \"messageid\"")
    b.append("        ]")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"messages\",")
    b.append("        \"schedule\",")
    b.append("        \"{messageid}\"")
    b.append("       ]")
    b.append("      }")
    b.append("     ]")
    b.append("    }")
    b.append("   },")
    b.append("   \"relations\": {")
    b.append("    \"ancestors\": [")
    b.append("     [")
    b.append("      \"schedule\"")
    b.append("     ]")
    b.append("    ]")
    b.append("   }")
    b.append("  },")
    b.append("  \"one_time_password\": {")
    b.append("   \"fields\": [")
    b.append("    {")
    b.append("     \"name\": \"destination\",")
    b.append("     \"short\": \"The phone number of the recipient.\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"length\",")
    b.append("     \"short\": \"The length of the generated passcode.\",")
    b.append("     \"type\": \"`$OBJECT`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"metadata\",")
    b.append("     \"short\": \"A JSON object of no longer than 1024 bytes, containing as many parameters as you wish, to store data for use in your application.\",")
    b.append("     \"type\": \"`$OBJECT`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"passcode\",")
    b.append("     \"short\": \"A passcode you supply for use in the message template.\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"sender\",")
    b.append("     \"short\": \"The sender of the message.\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"template\",")
    b.append("     \"short\": \"A template to use as the content for the message.\",")
    b.append("     \"type\": \"`$STRING`\"")
    b.append("    },")
    b.append("    {")
    b.append("     \"name\": \"validity\",")
    b.append("     \"short\": \"The length of time in seconds for which the generated passcode should be valid.\",")
    b.append("     \"type\": \"`$NUMBER`\"")
    b.append("    }")
    b.append("   ],")
    b.append("   \"name\": \"one_time_password\",")
    b.append("   \"op\": {")
    b.append("    \"create\": {")
    b.append("     \"input\": \"data\",")
    b.append("     \"name\": \"create\",")
    b.append("     \"points\": [")
    b.append("      {")
    b.append("       \"args\": {},")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"POST\",")
    b.append("       \"orig\": \"/otp/send\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"otp\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"send\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {},")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"otp\",")
    b.append("        \"send\"")
    b.append("       ]")
    b.append("      },")
    b.append("      {")
    b.append("       \"args\": {},")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"POST\",")
    b.append("       \"orig\": \"/otp/verify\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"otp\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"verify\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {},")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"otp\",")
    b.append("        \"verify\"")
    b.append("       ]")
    b.append("      }")
    b.append("     ]")
    b.append("    },")
    b.append("    \"load\": {")
    b.append("     \"input\": \"data\",")
    b.append("     \"name\": \"load\",")
    b.append("     \"points\": [")
    b.append("      {")
    b.append("       \"args\": {")
    b.append("        \"params\": [")
    b.append("         {")
    b.append("          \"kind\": \"param\",")
    b.append("          \"name\": \"messageid\",")
    b.append("          \"orig\": \"messageid\",")
    b.append("          \"reqd\": true,")
    b.append("          \"type\": \"`$STRING`\"")
    b.append("         }")
    b.append("        ]")
    b.append("       },")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"GET\",")
    b.append("       \"orig\": \"/otp/{messageid}\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"otp\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"var\": \"messageid\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"exist\": [")
    b.append("         \"messageid\"")
    b.append("        ]")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"otp\",")
    b.append("        \"{messageid}\"")
    b.append("       ]")
    b.append("      }")
    b.append("     ]")
    b.append("    }")
    b.append("   },")
    b.append("   \"relations\": {")
    b.append("    \"ancestors\": [")
    b.append("     [")
    b.append("      \"otp\"")
    b.append("     ]")
    b.append("    ]")
    b.append("   }")
    b.append("  },")
    b.append("  \"schedule\": {")
    b.append("   \"fields\": [],")
    b.append("   \"name\": \"schedule\",")
    b.append("   \"op\": {},")
    b.append("   \"relations\": {")
    b.append("    \"ancestors\": []")
    b.append("   }")
    b.append("  },")
    b.append("  \"swagger\": {")
    b.append("   \"fields\": [],")
    b.append("   \"name\": \"swagger\",")
    b.append("   \"op\": {},")
    b.append("   \"relations\": {")
    b.append("    \"ancestors\": []")
    b.append("   }")
    b.append("  },")
    b.append("  \"util\": {")
    b.append("   \"fields\": [],")
    b.append("   \"name\": \"util\",")
    b.append("   \"op\": {")
    b.append("    \"load\": {")
    b.append("     \"input\": \"data\",")
    b.append("     \"name\": \"load\",")
    b.append("     \"points\": [")
    b.append("      {")
    b.append("       \"args\": {")
    b.append("        \"params\": [")
    b.append("         {")
    b.append("          \"kind\": \"param\",")
    b.append("          \"name\": \"errorcode\",")
    b.append("          \"orig\": \"errorcode\",")
    b.append("          \"reqd\": true,")
    b.append("          \"type\": \"`$STRING`\"")
    b.append("         }")
    b.append("        ]")
    b.append("       },")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"GET\",")
    b.append("       \"orig\": \"/utils/errors/{errorcode}\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"utils\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"errors\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"var\": \"errorcode\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"exist\": [")
    b.append("         \"errorcode\"")
    b.append("        ]")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"utils\",")
    b.append("        \"errors\",")
    b.append("        \"{errorcode}\"")
    b.append("       ]")
    b.append("      },")
    b.append("      {")
    b.append("       \"args\": {},")
    b.append("       \"kind\": \"http\",")
    b.append("       \"method\": \"GET\",")
    b.append("       \"orig\": \"/utils/test\",")
    b.append("       \"segments\": [")
    b.append("        {")
    b.append("         \"lit\": \"utils\"")
    b.append("        },")
    b.append("        {")
    b.append("         \"lit\": \"test\"")
    b.append("        }")
    b.append("       ],")
    b.append("       \"select\": {")
    b.append("        \"$action\": \"test\"")
    b.append("       },")
    b.append("       \"transform\": {")
    b.append("        \"req\": \"`reqdata`\",")
    b.append("        \"res\": \"`body`\"")
    b.append("       },")
    b.append("       \"parts\": [")
    b.append("        \"utils\",")
    b.append("        \"test\"")
    b.append("       ]")
    b.append("      }")
    b.append("     ]")
    b.append("    }")
    b.append("   },")
    b.append("   \"relations\": {")
    b.append("    \"ancestors\": [")
    b.append("     [")
    b.append("      \"error\"")
    b.append("     ]")
    b.append("    ]")
    b.append("   }")
    b.append("  }")
    b.append(" }")
    b.append("}")
    b.toString
  }
}
