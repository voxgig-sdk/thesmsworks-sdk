// Generated direct-call tests for the batch entity (unit mode;
...
static BatchDirectSetup batch_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void batch_direct_load() {
  auto setup = batch_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-batch", "unit");
...
int main() {
  T_RUN(batch_direct_load);
  return sdktest::summary("batch_direct_test");
}
