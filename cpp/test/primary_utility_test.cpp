// Thesmsworks SDK — drives the primary utility functions against the shared
...
static res::Run& primaryRun() {
  static res::Run run =
      res::makeRunner(TEST_JSON_FILE, ThesmsworksSDK::testSDK())
          .runner("primary", Value::undef());
  return run;
}

static std::shared_ptr<ThesmsworksSDK> client() { return ThesmsworksSDK::testSDK(); }
...
static CtxPtr makeTestCtx(std::shared_ptr<ThesmsworksSDK> client_, UtilityPtr utility) {
  CtxSpec cs;
  cs.setOpname("load");
...
static CtxPtr makeTestFullCtx(std::shared_ptr<ThesmsworksSDK> client_, UtilityPtr utility) {
...
static void fetcherLive() {
  ...
  auto liveClient = std::make_shared<ThesmsworksSDK>(opts);
...
static void fetcherBlockedTestMode() {
  ...
  Value opts = fhMap({{"system", fhMap({{"fetch", Value(fetchFn)}})}});
  auto blockedClient = std::make_shared<ThesmsworksSDK>(opts);
...
static void makeSpecBasic() {
  ...
  auto specClient = ThesmsworksSDK::testSDK(Value::undef(), setupOpts);
...
static void prepareAuthBasic() {
  ...
  auto authClient = ThesmsworksSDK::testSDK(Value::undef(), setupOpts);
