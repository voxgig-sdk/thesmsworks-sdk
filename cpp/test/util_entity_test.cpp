...
static UtilSetup util_basic_setup(const Value& extra) {
  load_env_local();

  std::string entity_data_file = "../.sdk/test/entity/util/UtilTestData.json";
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "THESMSWORKS_TEST_UTIL_ENTID"));
...
static void util_entity_instance() {
  auto testsdk = ThesmsworksSDK::testSDK();
  auto ent = testsdk->util();
  ASSERT_EQ(ent->getName(), std::string("util"), "entity name");
}

static void util_entity_stream() {
  // stream() runs the list op through the full pipeline and returns the
  // result items. Seed two entities via test mode; with the streaming feature
  // active it yields the feature's incremental items, else it falls back to
  // the materialised items — either way every item is yielded.
  Value seed = vmap({{"entity", vmap({{"util", vmap({
...
static void util_entity_basic() {
  ...
  Value util_ref01_data_raw = Helpers::toMapAny(Struct::getpath(setup.data, {"existing", "util"}));
  Value util_ref01_data = vmap();
  {
    std::vector<Value> its = Struct::items(util_ref01_data_raw);
  ...
  Value util_ref01_data_dt0_loaded = util_ref01_ent->load(util_ref01_match_dt0, Value::undef());
