// Generated direct-call tests for the credit entity (unit mode;
...
static CreditDirectSetup credit_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void credit_direct_load() {
  auto setup = credit_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-credit", "unit");
...
int main() {
  T_RUN(credit_direct_load);
  return sdktest::summary("credit_direct_test");
}
