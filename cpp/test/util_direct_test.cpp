// Generated direct-call tests for the util entity (unit mode;
...
static UtilDirectSetup util_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void util_direct_load() {
  auto setup = util_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-util", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;

  Value params = vmap();
  setp(params, "errorcode", Value("direct01"));

  Value result = client->direct(vmap({
    {"path", Value("utils/errors/{errorcode}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(util_direct_load);
  return sdktest::summary("util_direct_test");
}
