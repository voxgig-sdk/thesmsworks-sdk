...
pub const ThesmsworksSDK = struct {
    ...
    pub fn new_with(options: Value, extend: []const Feature) *ThesmsworksSDK {
    ...
    fn op_allowed(self: *ThesmsworksSDK, op: []const u8) bool {
    ...
    fn op_denied(self: *ThesmsworksSDK, op: []const u8) Value {
        const allow: []const u8 = switch (h.getpath(&.{ "allow", "op" }, self.options)) {
            .string => |s| s,
            else => "",
        };
    ...
    // gate by passing it.
    fn raw_request(self: *ThesmsworksSDK, fetchargs_in: Value) Value {
    ...
    pub fn graphql(
        self: *ThesmsworksSDK, query: []const u8, variables: Value, ctrl: Value,
    ) Value {
        ...
        if (errors == .array and 0 < errors.array.data.items.len) {
            ...
            const msg = std.fmt.allocPrint(h.A(),
                "ThesmsworksSDK: graphql: {s}", .{m}) catch "";
