// Swagger entity client (generated — mirrors the go/rust Entity fragment).
...
pub const SwaggerEntity = struct {
    name: []const u8 = "swagger",
    ...
    pub fn new(client: *sdk.ThesmsworksSDK, entopts_in: Value) *SwaggerEntity {
        ...
        const e = h.A().create(SwaggerEntity) catch unreachable;
        e.* = .{
            .name = "swagger",
            .client = client,
    ...
    fn ent_ctx(self: *SwaggerEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *SwaggerEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *SwaggerEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *SwaggerEntity, ctx: *Context, post_done: *const fn (*SwaggerEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *SwaggerEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *SwaggerEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *SwaggerEntity, ctx: *Context, post_done: *const fn (*SwaggerEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *SwaggerEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *SwaggerEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return SwaggerEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *SwaggerEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *SwaggerEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *SwaggerEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *SwaggerEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *SwaggerEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("list", self.name) };
    }

    pub fn create(self: *SwaggerEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *SwaggerEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *SwaggerEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
