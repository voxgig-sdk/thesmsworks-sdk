// Message entity client (generated — mirrors the go/rust Entity fragment).
...
pub const MessageEntity = struct {
    ...
    pub fn new(client: *sdk.ThesmsworksSDK, entopts_in: Value) *MessageEntity {
        ...
        const e = h.A().create(MessageEntity) catch unreachable;
        e.* = .{
            .name = "message",
            .client = client,
    ...
    fn ent_ctx(self: *MessageEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *MessageEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *MessageEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *MessageEntity, ctx: *Context, post_done: *const fn (*MessageEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *MessageEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *MessageEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *MessageEntity, ctx: *Context, post_done: *const fn (*MessageEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *MessageEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *MessageEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return MessageEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *MessageEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *MessageEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *MessageEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn create(self: *MessageEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *MessageEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *MessageEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
