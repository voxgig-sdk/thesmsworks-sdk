// Util entity client (generated — mirrors the go/rust Entity fragment).
...
pub const UtilEntity = struct {
    ...
    pub fn new(client: *sdk.ThesmsworksSDK, entopts_in: Value) *UtilEntity {
        ...
        const e = h.A().create(UtilEntity) catch unreachable;
        e.* = .{
            .name = "util",
            .client = client,
    ...
    fn ent_ctx(self: *UtilEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *UtilEntity, ctx: *Context) OpResult {
    ...
    pub fn stream(self: *UtilEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *UtilEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return UtilEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *UtilEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *UtilEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *UtilEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
