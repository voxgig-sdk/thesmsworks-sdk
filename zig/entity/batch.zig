// Batch entity client (generated — mirrors the go/rust Entity fragment).
...
pub const BatchEntity = struct {
    ...
    pub fn new(client: *sdk.ThesmsworksSDK, entopts_in: Value) *BatchEntity {
        ...
        const e = h.A().create(BatchEntity) catch unreachable;
        e.* = .{
            .name = "batch",
            .client = client,
    ...
    fn ent_ctx(self: *BatchEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *BatchEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *BatchEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *BatchEntity, ctx: *Context, post_done: *const fn (*BatchEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *BatchEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *BatchEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *BatchEntity, ctx: *Context, post_done: *const fn (*BatchEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *BatchEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *BatchEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return BatchEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *BatchEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *BatchEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *BatchEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *BatchEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *BatchEntity, ctx: *Context) void {
    ...
    pub fn list(self: *BatchEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("list", self.name) };
    }

    pub fn create(self: *BatchEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *BatchEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *BatchEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
