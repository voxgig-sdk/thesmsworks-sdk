// OneTimePassword entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OneTimePasswordEntity = struct {
    name: []const u8 = "one_time_password",
    ...
    pub fn new(client: *sdk.ThesmsworksSDK, entopts_in: Value) *OneTimePasswordEntity {
        ...
        const e = h.A().create(OneTimePasswordEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OneTimePasswordEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OneTimePasswordEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OneTimePasswordEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OneTimePasswordEntity, ctx: *Context, post_done: *const fn (*OneTimePasswordEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OneTimePasswordEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OneTimePasswordEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OneTimePasswordEntity, ctx: *Context, post_done: *const fn (*OneTimePasswordEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OneTimePasswordEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OneTimePasswordEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OneTimePasswordEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OneTimePasswordEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OneTimePasswordEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OneTimePasswordEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OneTimePasswordEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *OneTimePasswordEntity, ctx: *Context) void {
    ...
    pub fn list(self: *OneTimePasswordEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *OneTimePasswordEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *OneTimePasswordEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *OneTimePasswordEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OneTimePasswordEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
