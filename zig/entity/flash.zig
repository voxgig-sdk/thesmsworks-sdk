// Flash entity client (generated — mirrors the go/rust Entity fragment).
...
pub const FlashEntity = struct {
    ...
    pub fn new(client: *sdk.ThesmsworksSDK, entopts_in: Value) *FlashEntity {
        ...
        const e = h.A().create(FlashEntity) catch unreachable;
        e.* = .{
            .name = "flash",
            .client = client,
    ...
    fn ent_ctx(self: *FlashEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *FlashEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *FlashEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *FlashEntity, ctx: *Context, post_done: *const fn (*FlashEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *FlashEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *FlashEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *FlashEntity, ctx: *Context, post_done: *const fn (*FlashEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *FlashEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *FlashEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return FlashEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *FlashEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *FlashEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *FlashEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *FlashEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *FlashEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("list", self.name) };
    }

    pub fn create(self: *FlashEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *FlashEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *FlashEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
