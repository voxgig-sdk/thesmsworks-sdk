// Credit entity client (generated — mirrors the go/rust Entity fragment).
...
pub const CreditEntity = struct {
    name: []const u8 = "credit",
    ...
    pub fn new(client: *sdk.ThesmsworksSDK, entopts_in: Value) *CreditEntity {
        ...
        const e = h.A().create(CreditEntity) catch unreachable;
        e.* = .{
            .name = "credit",
            .client = client,
    ...
    fn ent_ctx(self: *CreditEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *CreditEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *CreditEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *CreditEntity, ctx: *Context, post_done: *const fn (*CreditEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *CreditEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *CreditEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *CreditEntity, ctx: *Context, post_done: *const fn (*CreditEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *CreditEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *CreditEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return CreditEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *CreditEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *CreditEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *CreditEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *CreditEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *CreditEntity, ctx: *Context) void {
    ...
    pub fn list(self: *CreditEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("list", self.name) };
    }

    pub fn create(self: *CreditEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *CreditEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *CreditEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
