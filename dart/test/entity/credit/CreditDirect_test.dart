...
void tests() {
  describe('CreditDirect', () {
    ...
    test('direct-load-credit', (t) async {
      final setup = directSetup({'id': 'direct01'});
      if (maybeSkipControl(
          t, 'direct', 'direct-load-credit', true == setup['live'])) {
        return;
      }
...
Map<String, dynamic> directSetup([dynamic mockres]) {
  ...
  final env = envOverride({
    'THESMSWORKS_TEST_CREDIT_ENTID': <String, dynamic>{},
  ...
  if (live) {
    ...
    dynamic idmap = env['THESMSWORKS_TEST_CREDIT_ENTID'];
