...
void tests() {
  describe('UtilDirect', () {
    ...
    test('direct-load-util', (t) async {
      final setup = directSetup({'id': 'direct01'});
      if (maybeSkipControl(
          t, 'direct', 'direct-load-util', true == setup['live'])) {
        return;
      }
      if (skipIfMissingIds(t, setup, ["errorcode01"])) {
        return;
      }
...
Map<String, dynamic> directSetup([dynamic mockres]) {
  ...
  final env = envOverride({
    'THESMSWORKS_TEST_UTIL_ENTID': <String, dynamic>{},
  ...
  if (live) {
    ...
    dynamic idmap = env['THESMSWORKS_TEST_UTIL_ENTID'];
