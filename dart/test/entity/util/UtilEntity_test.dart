  ...
  describe('UtilEntity', () {
    ...
    test('basic', (t) async {
      ...
      for (final op in ['load']) {
        if (maybeSkipControl(t, 'entityOp', 'util.' + op, live)) {
          return;
        }
      }
      ...
      final util_ref01_data =
          (setup['data']['existing']['util'] as Map).values.first;

      // LOAD: skipped — no entity id field and load requires path params.
      // Entity-var is declared here so later flow steps still compile.
      final util_ref01_ent = client.Util();
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  final idmapEnvVal =
      Platform.environment['THESMSWORKS_TEST_UTIL_ENTID'];
