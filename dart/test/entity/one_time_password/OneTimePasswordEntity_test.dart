...
void tests() {
  describe('OneTimePasswordEntity', () {
    test('instance', (t) async {
      final testsdk = ThesmsworksSDK.test();
      final ent = testsdk.OneTimePassword();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.OneTimePassword();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create', 'load']) {
        if (maybeSkipControl(t, 'entityOp', 'one_time_password.' + op, live)) {
          return;
        }
      }
      ...
      // CREATE
      final one_time_password_ref01_ent = client.OneTimePassword();
      dynamic one_time_password_ref01_data = setup['data']['new']['one_time_password']['one_time_password_ref01'];

      one_time_password_ref01_data = await one_time_password_ref01_ent.create(one_time_password_ref01_data);
      ok(null != one_time_password_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  final idmapEnvVal =
      Platform.environment['THESMSWORKS_TEST_ONE_TIME_PASSWORD_ENTID'];
