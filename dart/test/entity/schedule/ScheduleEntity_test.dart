...
void tests() {
  describe('ScheduleEntity', () {
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.Schedule();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in []) {
        if (maybeSkipControl(t, 'entityOp', 'schedule.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set THESMSWORKS_TEST_SCHEDULE_ENTID JSON to run live');
        return;
      }
      ...
      final schedule_ref01_data =
          (setup['data']['existing']['schedule'] as Map).values.first;
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['schedule01', 'schedule02', 'schedule03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['THESMSWORKS_TEST_SCHEDULE_ENTID'];
