...
dynamic makeError(dynamic ctx, [dynamic err]) {
  ...
  final msg = 'ThesmsworksSDK: ' + opname.toString() + ': ' + errmsg(err);

  ThesmsworksError sdkerr;
  if (err is ThesmsworksError) {
    sdkerr = err;
    sdkerr.message = clean(ctx, msg);
  } else {
    final code = errcode(err);
    sdkerr = ThesmsworksError('' == code ? 'unknown' : code, clean(ctx, msg), ctx);
  }
