...
// `utility` (ThesmsworksTypes.dart) — exporting both from this library makes
// the name ambiguous and NO dart SDK for such an API compiles:
//
//   Error: 'Utility' is exported from both 'lib/ThesmsworksTypes.dart'
...
class ThesmsworksSDK {
  ...
  dynamic _opDenied(String op) {
    ...
    return {
      'ok': false,
      'err': Exception('ThesmsworksSDK: $op: operation not allowed by'
  ...
  Future<dynamic> graphql(String query,
    ...
    if (errors is List && 0 < errors.length) {
      ...
      res['ok'] = false;
      res['err'] = Exception('ThesmsworksSDK: graphql: '
