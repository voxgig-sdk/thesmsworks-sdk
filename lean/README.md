# Thesmsworks Lean SDK



The Lean SDK for the Thesmsworks API — an entity-oriented client
...
[Releases](https://github.com/voxgig-sdk/thesmsworks-sdk/releases)) or from a source checkout. The runtime is
...
def main : IO Unit := do
  let ctx ← mkCtx
  (do
    let opts ← newMap #[("base", .str "https://api.example.com"), ("apikey", .str (← IO.getEnv "THESMSWORKS").getD "")]
    let sdk ← Sdk.newSdk opts
    let result ← Batch.load sdk (← emptyMap) (← emptyMap)
