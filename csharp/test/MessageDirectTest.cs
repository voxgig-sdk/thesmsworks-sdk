// message direct API tests (generated from the API model).
...
public class MessageDirectTest
{
    ...
    {
        var setup = MessageDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-message", _mode);
        if (_shouldSkip)
        {
    ...
    private class MessageDirectSetupResult
    {
    ...
    private static MessageDirectSetupResult MessageDirectSetup(object? mockres)
    {
        ...
        {
            ["THESMSWORKS_TEST_MESSAGE_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["THESMSWORKS_TEST_MESSAGE_ENTID"];
            ...
            return new MessageDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
