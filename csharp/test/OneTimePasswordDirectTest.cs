// one_time_password direct API tests (generated from the API model).
...
public class OneTimePasswordDirectTest
{
    [Fact]
    public void DirectLoad()
    {
        var setup = OneTimePasswordDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-one_time_password", _mode);
        if (_shouldSkip)
        {
        ...
        if (setup.Live)
        {
            foreach (var _liveKey in new[] { "messageid01" })
            {
    ...
    private class OneTimePasswordDirectSetupResult
    {
    ...
    private static OneTimePasswordDirectSetupResult OneTimePasswordDirectSetup(object? mockres)
    {
        ...
        {
            ["THESMSWORKS_TEST_ONE_TIME_PASSWORD_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["THESMSWORKS_TEST_ONE_TIME_PASSWORD_ENTID"];
            ...
            return new OneTimePasswordDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
