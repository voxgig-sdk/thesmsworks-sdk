// batch_message entity test - basic flow (generated from the API model).
...
public class BatchMessageEntityTest
{
    ...
    public void Basic()
    {
        var setup = BatchMessageBasicSetup(null);
        ...
        foreach (var _op in new[] { "create", "remove" })
        {
            var (_shouldSkip, _) = TestRunner.IsControlSkipped(
                "entityOp", "batch_message." + _op, _mode);
            if (_shouldSkip)
            {
        ...
        // and 4xx; set THESMSWORKS_TEST_BATCH_MESSAGE_ENTID JSON to run live.
        ...
        // CREATE
        var batchMessageRef01Ent = client.BatchMessage();
        var batchMessageRef01Data = Helpers.ToMapAny(StructUtils.GetProp(
            StructUtils.GetPath(setup.Data, StructUtils.Jt("new", "batch_message")),
            "batch_message_ref01"));
        ...
        // REMOVE
        var batchMessageRef01MatchRm0 = new Dictionary<string, object?>
        {
    ...
    private static EntityTestSetup BatchMessageBasicSetup(
        Dictionary<string, object?>? extra)
    {
        TestRunner.LoadEnvLocal();

        var entityDataFile = Path.Combine(TestRunner.TestDir(),
            "..", "..", ".sdk", "test", "entity", "batch_message",
            "BatchMessageTestData.json");
        ...
        var entityData = StructRunner.ConvertElement(entityDataEl)
            ...
            ?? throw new InvalidOperationException(
                "failed to parse batch_message test data");
        ...
        var idmapResolved = Helpers.ToMapAny(env["THESMSWORKS_TEST_BATCH_MESSAGE_ENTID"])
