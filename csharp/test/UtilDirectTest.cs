// util direct API tests (generated from the API model).
...
public class UtilDirectTest
{
    [Fact]
    public void DirectLoad()
    {
        var setup = UtilDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-util", _mode);
        if (_shouldSkip)
        {
        ...
        if (setup.Live)
        {
            foreach (var _liveKey in new[] { "errorcode01" })
            {
    ...
    private class UtilDirectSetupResult
    {
    ...
    private static UtilDirectSetupResult UtilDirectSetup(object? mockres)
    {
        ...
        {
            ["THESMSWORKS_TEST_UTIL_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["THESMSWORKS_TEST_UTIL_ENTID"];
            ...
            return new UtilDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
