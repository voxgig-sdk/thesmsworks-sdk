...
class CreditEntityTest {
  ...
  fun basic() {
    ...
    val creditRef01DataRaw = Struct.items(Helpers.toMapAny(
        Struct.getpath(setup.data, "existing.credit")))
    val creditRef01Data: MutableMap<String, Any?> = if (creditRef01DataRaw.isEmpty())
        linkedMapOf() else (Helpers.toMapAny(creditRef01DataRaw[0][1]) ?: linkedMapOf())

    // LOAD
    val creditRef01Ent = client.credit(null)
    val creditRef01MatchDt0 = linkedMapOf<String, Any?>()
  ...
  companion object {
    fun creditBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read credit test data: " + e.message, e)
      }
      ...
      envm["THESMSWORKS_TEST_CREDIT_ENTID"] = idmap
