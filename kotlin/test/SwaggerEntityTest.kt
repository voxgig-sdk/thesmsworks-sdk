...
class SwaggerEntityTest {
  ...
  @Test
  fun basic() {
    val setup = swaggerBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf()) {
      val reason = RunnerSupport.skipReason("entityOp", "swagger.$op", mode)
    ...
    val swaggerRef01DataRaw = Struct.items(Helpers.toMapAny(
        Struct.getpath(setup.data, "existing.swagger")))
    val swaggerRef01Data: MutableMap<String, Any?> = if (swaggerRef01DataRaw.isEmpty())
        linkedMapOf() else (Helpers.toMapAny(swaggerRef01DataRaw[0][1]) ?: linkedMapOf())

  }

  companion object {
    fun swaggerBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read swagger test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("THESMSWORKS_TEST_SWAGGER_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["THESMSWORKS_TEST_SWAGGER_ENTID"] = idmap
