...
/// ThesmsworksEntity: the full CRUD contract every generated entity
/// implements. Ops the API spec doesn't define are runtime-error stubs.
pub trait ThesmsworksEntity: Entity + Sized {
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, ThesmsworksError>;
    fn create(self: &Rc<Self>, reqdata: Value, ctrl: Value) -> Result<Rc<Self>, ThesmsworksError>;
    fn update(self: &Rc<Self>, reqdata: Value, ctrl: Value) -> Result<Rc<Self>, ThesmsworksError>;
...
pub type FetcherFn =
    Rc<dyn Fn(&Rc<Context>, &str, &Value) -> Result<Value, ThesmsworksError>>;
