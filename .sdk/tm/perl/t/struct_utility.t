...
my $runner = ThesmsworksOmni::make_runner($corpus_path, $client);
my $R = $runner->('struct');
...
my $NULLMARK = ThesmsworksOmni::NULLMARK();
...
{
  my $td = ThesmsworksOmni::tostruct(
...
{
  my $basic = ThesmsworksOmni::tostruct(
...
grun('inject.string', $inject_spec->{string},
  sub { $S->{inject}->($_[0]{val}, $_[0]{store},
    { modify => \&ThesmsworksOmni::null_modifier }) });
