...
#   live blessed context with ThesmsworksOmni::livectx before calling a
...
my $_runner = ThesmsworksOmni::make_runner($TEST_JSON, ThesmsworksSDK->test(undef, undef));
...

{
  ...
  our @ISA = ('ThesmsworksBaseFeature');

  sub new {
    my ($class, $fn) = @_;
    my $self = ThesmsworksBaseFeature::new($class);
    $self->{hook_fn} = $fn;
    return $self;
  }
...
sub _err_from_map {
  ...
  return ThesmsworksError->new($code, $msg);
}
...
sub _def_setup {
  ...
  return ThesmsworksOmni::tostruct(Voxgig::Omni::Util::clone($setup));
}
...
runsection('done', sub {
  my ($view) = @_;
  return $utility->{done}->(ThesmsworksOmni::livectx($view));
});
...
runsection('makeError', sub {
  my ($view, $errmap) = @_;
  my $ctx = ThesmsworksOmni::livectx($view);
...

{
  ...
  $ctx->{result} = ThesmsworksResult->new({
    'ok' => 0,
    'resdata' => { 'id' => 'safe01' },
  });
...

{
  ...
  my $feature = ThesmsworksBaseFeature->new;
...
{
  my $hook_client = ThesmsworksSDK->test(undef, undef);
...
{
  my $init_client = ThesmsworksSDK->test(undef, undef);
...
{
  my $calls = [];
  my $live_client = ThesmsworksSDK->new({
...
{
  my $blocked_client = ThesmsworksSDK->new({
    'base' => 'http://localhost:8080',
    'system' => {
...
runsection('makeResponse', sub {
  my ($view) = @_;
  return _unwrap($utility->{make_response}->(ThesmsworksOmni::livectx($view)));
});
...
{
  my $ctx = make_test_full_ctx($client, $utility);
  $ctx->{spec} = ThesmsworksSpec->new({ 'step' => 'start' });
  $ctx->{result} = undef;
  my (undef, $err) = $utility->{make_result}->($ctx);
...
{
  my $spec_client = ThesmsworksSDK->test(undef, _def_setup('makeSpec'));
...
runsection('makeUrl', sub {
  my ($view) = @_;
  my $ctx = ThesmsworksOmni::livectx($view);
  $ctx->{result} = ThesmsworksResult->new({}) unless $ctx->{result};
...
runsection('operator', sub {
  my ($vin) = @_;
  $vin = {} unless ref($vin) eq 'HASH';
  my $op = ThesmsworksOperation->new($vin);
  return {
    'entity' => $op->{entity},
    'name' => $op->{name},
    'input' => $op->{input},
...
runsection('param', sub {
  my ($view, $paramdef) = @_;
  return $utility->{param}->(ThesmsworksOmni::livectx($view), $paramdef);
});
...
{
  my $auth_client = ThesmsworksSDK->test(undef, _def_setup('prepareAuth'));
...
runsection('prepareBody', sub {
  my ($view) = @_;
  return $utility->{prepare_body}->(ThesmsworksOmni::livectx($view));
});
...
runsection('prepareHeaders', sub {
  my ($view) = @_;
  return $utility->{prepare_headers}->(ThesmsworksOmni::livectx($view));
});
...
runsection('prepareMethod', sub {
  my ($view) = @_;
  return $utility->{prepare_method}->(ThesmsworksOmni::livectx($view));
});
...
runsection('prepareParams', sub {
  my ($view) = @_;
  return $utility->{prepare_params}->(ThesmsworksOmni::livectx($view));
});
...
runsection('preparePath', sub {
  my ($view) = @_;
  return $utility->{prepare_path}->(ThesmsworksOmni::livectx($view));
});
...
runsection('prepareQuery', sub {
  my ($view) = @_;
  return $utility->{prepare_query}->(ThesmsworksOmni::livectx($view));
});
...
runsection('resultBasic', sub {
  my ($view) = @_;
  my $result = $utility->{result_basic}->(ThesmsworksOmni::livectx($view));
...
runsection('resultBody', sub {
  my ($view) = @_;
  return $utility->{result_body}->(ThesmsworksOmni::livectx($view));
});
...
runsection('resultHeaders', sub {
  my ($view) = @_;
  return $utility->{result_headers}->(ThesmsworksOmni::livectx($view));
});
...
runsection('transformRequest', sub {
  my ($view) = @_;
  return $utility->{transform_request}->(ThesmsworksOmni::livectx($view));
});
...
runsection('transformResponse', sub {
  my ($view) = @_;
  return $utility->{transform_response}->(ThesmsworksOmni::livectx($view));
});
