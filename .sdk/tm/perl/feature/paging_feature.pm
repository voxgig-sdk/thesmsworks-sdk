# Thesmsworks SDK paging feature
#
...
package ThesmsworksPagingFeature;
...
sub init {
  ...
  $self->{active} = ThesmsworksHelpers::is_true($self->{options}{active});
  return;
}
...
sub PreRequest {
  ...
  if ('graphql' eq (ThesmsworksHelpers::gp($ctx->{point}, 'kind') // '')) {
...
sub _graphql_pre_request {
  ...
  my %declared;
  my $varlist = ThesmsworksHelpers::gpath($ctx->{point}, 'graphql.vars');
  if (ref($varlist) eq 'ARRAY') {
    for my $v (@$varlist) {
      my $name = ThesmsworksHelpers::gp($v, 'name');
...
sub PreResult {
  ...
  my $page = ThesmsworksHelpers::gpath($ctx->{point}, 'graphql.page');
  if (ref($page) eq 'HASH' && Voxgig::Struct::ismap($body)) {
    ...
    if (defined $connpath && '' ne $connpath) {
      my $sub = ThesmsworksHelpers::gpath($body, $connpath);
      $conn = $sub if defined $sub;
    }

    my $cursorpath = $page->{cursor};
    if (defined $cursorpath && '' ne $cursorpath) {
      my $cursor = ThesmsworksHelpers::gpath($conn, $cursorpath);
      $paging->{cursor} = $cursor if defined $cursor;
    }

    my $morepath = $page->{more};
    if (defined $morepath && '' ne $morepath) {
      my $more = ThesmsworksHelpers::gpath($conn, $morepath);
      if (ThesmsworksHelpers::is_true($more) || ThesmsworksHelpers::is_false($more)) {
        $paging->{hasMore} = ThesmsworksHelpers::is_true($more) ? 1 : 0;
        $explicit_more = 1;
      }
    }
  }
  ...
  if (Voxgig::Struct::ismap($body)) {
    my $bnext = ThesmsworksHelpers::gp($body, 'next');
    $paging->{next} = defined $paging->{next} ? $paging->{next} : $bnext
      if defined $bnext;
    my $bcursor = ThesmsworksHelpers::gp($body, 'cursor');
    $paging->{cursor} = $bcursor if defined $bcursor;
    my $bnextcursor = ThesmsworksHelpers::gp($body, 'nextCursor');
    $paging->{cursor} = $bnextcursor if defined $bnextcursor;
    my $bhasmore = $body->{hasMore};
    if (ThesmsworksHelpers::is_true($bhasmore) || ThesmsworksHelpers::is_false($bhasmore)) {
      $paging->{hasMore} = ThesmsworksHelpers::is_true($bhasmore) ? 1 : 0;
      $explicit_more = 1;
    }
  }
