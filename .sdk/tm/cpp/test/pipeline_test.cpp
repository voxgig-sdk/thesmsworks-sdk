// Thesmsworks SDK — direct unit tests for the operation-pipeline utilities
...
static std::shared_ptr<ThesmsworksSDK> plClient(const Value& sdkopts) {
  return ThesmsworksSDK::testSDK(Value::undef(), sdkopts);
}

static CtxPtr plCtx(std::shared_ptr<ThesmsworksSDK> client, UtilityPtr utility, const Value& ctrl) {
  CtxSpec cs;
  cs.setOpname("load");
...
static std::string names(std::shared_ptr<ThesmsworksSDK> client) {
  std::string out;
