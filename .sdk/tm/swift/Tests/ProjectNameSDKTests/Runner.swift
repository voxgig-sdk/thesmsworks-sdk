// Thesmsworks SDK test runner - shared infrastructure for the generated test
...
enum SdkRunner {
  ...
  // three dirs from this file (swift/Tests/ThesmsworksSDKTests -> project).
  static func testJsonPath(_ file: String = #filePath) -> String {
    let dir = URL(fileURLWithPath: file)
      .deletingLastPathComponent()  // ThesmsworksSDKTests
  ...
  static func makeCtxFromMap(_ ctxmapIn: VMap?, _ client: ThesmsworksSDK?, _ utility: Utility?)
    -> Context
  {
    ...
    if let resMap = ctxmap.entries["result"]?.asMap {
      ...
      if let errMap = resMap.entries["err"]?.asMap, let msg = errMap.entries["message"]?.asString {
        ctx.result!.err = ThesmsworksError("", msg, nil)
      }
    }
  ...
  static func fixCtx(_ ctx: Context, _ client: ThesmsworksSDK) {
  ...
  static func errFromMap(_ m: VMap?) -> Error? {
    ...
    return ThesmsworksError(code, msg, nil)
  }
}
