...
enum OmniResolver {
  ...
  private final class ProviderBox {
    var clients: [ObjectIdentifier: ThesmsworksSDK] = [:]
  ...
  private static func register(_ provider: Omni.Provider, _ client: ThesmsworksSDK,
    _ isdef: Bool)
  {
    providers.lock.lock()
  ...
  private static func defclient(_ provider: Omni.Provider) -> ThesmsworksSDK? {
    providers.lock.lock()
    defer { providers.lock.unlock() }
  ...
  private static func sdkProvider(_ client: ThesmsworksSDK, _ isdef: Bool = false)
    -> Omni.Provider
  {
    ...
    provider.errify = { err in
      if let sdkerr = err as? ThesmsworksError {
        var entries: [(String, Omni.Json)] = [
          ("name", .str("ThesmsworksError")),
  ...
  final class Run {
    ...
    let client: ThesmsworksSDK

    private let pack: Omni.RunPack

    fileprivate init(_ pack: Omni.RunPack, _ client: ThesmsworksSDK) {
      self.pack = pack
  ...
  /// itself).
  static func makeRunner(_ testfile: String, _ client: ThesmsworksSDK) throws
    -> NamedRunner
  {
  ...
  static func makeRunner(_ spec: Value, _ client: ThesmsworksSDK) throws
    -> NamedRunner
  {
    return try namedRunner(try Omni.makeRunner(tojson(spec), sdkProvider(client)), client)
  }

  private static func namedRunner(_ runner: Omni.Runner, _ client: ThesmsworksSDK)
    -> NamedRunner
  {
  ...
  static func omniCtx(_ arg: Value, _ client: ThesmsworksSDK, _ utility: Utility)
    -> Context
  {
