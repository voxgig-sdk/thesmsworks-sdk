# Thesmsworks SDK generic entity behaviour
#
...
defmodule Thesmsworks.EntityBase do
  alias Voxgig.Struct, as: S
  alias Thesmsworks.Helpers, as: H
  alias Thesmsworks.{Context, Utility, Pipeline}
  ...
  def construct(module, client, name, entopts \\ nil) do
    ...
    S.setprop(ent, "_utility", Thesmsworks.get_utility(client))
    S.setprop(ent, "_entopts", entopts)
    S.setprop(ent, "_data", S.jm([]))
    S.setprop(ent, "_match", S.jm([]))

    entctx =
      Context.new(S.jm(["entity", ent, "entopts", entopts]), Thesmsworks.get_root_ctx(client))
