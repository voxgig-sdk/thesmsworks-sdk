...
module ThesmsworksUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ThesmsworksResponse.new({})
    ...
    if fetch_err
      ...
      response = ThesmsworksResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ThesmsworksResponse.new(fetched)
    else
