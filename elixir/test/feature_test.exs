...
# Thesmsworks.FeatureHarness — the same hook order and short-circuit rules as
...
defmodule Thesmsworks.FeatureTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias Thesmsworks.Helpers, as: H
  alias Thesmsworks.FeatureHarness, as: FH
    ...
    if has?("retry") do
      ...
      {server, calls} =
        FH.recording_server(fn n, _fd ->
          if n < 3,
            do: {nil, Thesmsworks.Error.new("boom", "boom")},
  ...
  test "retry exhausted transport error surfaces" do
    if has?("retry") do
      clock = FH.make_clock()
      {server, calls} = FH.recording_server(fn _n, _fd -> {nil, Thesmsworks.Error.new("boom", "boom")} end)
