...
defmodule Thesmsworks do
  alias Voxgig.Struct, as: S
  alias Thesmsworks.Helpers, as: H
  alias Thesmsworks.{Utility, Context, Spec}
  ...
  def new(options \\ nil) do
    ...
    config = Thesmsworks.Config.shared_config()
    ...
    if S.islist(feature_order) and S.size(feature_order) > 0 do
      Enum.each(0..(S.size(feature_order) - 1), fn i ->
        ...
        if fom != nil and S.getprop(fom, "active") == true do
          Utility.feature_add(rootctx, Thesmsworks.Features.make_feature(fname))
        end
      end)
    end
      ...
      _ ->
        ...
        cond do
          ...
          S.ismap(fetched) ->
            ...
            content_length =
              if S.ismap(headers), do: Thesmsworks.Feature.header_get(headers, "content-length"), else: nil
  ...
  def graphql(client, query, variables \\ nil, ctrl \\ nil) do
    if not op_allowed?(client, "graphql") do
      ...
      if count == 0 do
        ...
        S.setprop(res, "err", "ThesmsworksSDK: graphql: " <> msg)
  ...
  @doc "Entity factory for batch."
  def batch(client, entopts \\ nil) do
    Thesmsworks.Entity.Batch.new(client, entopts)
  end

  @doc "Entity factory for batch_message."
  def batch_message(client, entopts \\ nil) do
    Thesmsworks.Entity.BatchMessage.new(client, entopts)
  end

  @doc "Entity factory for credit."
  def credit(client, entopts \\ nil) do
  ...
  def util(client, entopts \\ nil) do
    Thesmsworks.Entity.Util.new(client, entopts)
  end
