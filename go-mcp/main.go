// Command Thesmsworks-mcp is an MCP server exposing the Thesmsworks
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Thesmsworks_list  — list records of an entity
//	Thesmsworks_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: THESMSWORKS_APIKEY carries the API key and
	// THESMSWORKS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("THESMSWORKS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("THESMSWORKS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewThesmsworksSDK(opts)
