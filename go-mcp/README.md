...
[MCP](https://modelcontextprotocol.io) server exposing the Thesmsworks SDK as
two agent tools — `thesmsworks_list` and `thesmsworks_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/thesmsworks-mcp)
make build

# 2. Provide credentials via the environment
export THESMSWORKS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user thesmsworks \
  -- /absolute/path/to/thesmsworks-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./thesmsworks-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// thesmsworks_list: first page of records
{ "entity": "batch" }
{ "entity": "batch", "query": { } }

// thesmsworks_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/thesmsworks-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user thesmsworks \
     -- "$PWD"/dist/*/thesmsworks-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `thesmsworks_list` and `thesmsworks_load` tools now appear
   in new sessions. Ask the agent to *"list batch using thesmsworks"*
   and it calls `thesmsworks_list` with `{"entity":"batch"}`.
...
```sh
export THESMSWORKS_APIKEY=sk_live_xxx            # API key
export THESMSWORKS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `thesmsworks_list` tool
...
### Call the `thesmsworks_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `thesmsworks_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `thesmsworks_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `THESMSWORKS_APIKEY` | API key sent with every request. |
| `THESMSWORKS_BASE` | Optional override of the API base URL. |
...
```sh
./thesmsworks-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"thesmsworks_load","arguments":{"entity":"batch","query":{"id":1}}}}'
```
