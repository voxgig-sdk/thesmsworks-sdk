...
MCP server exposing the Thesmsworks SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `thesmsworks_list` | `entity`, optional `query` map | First page of records as JSON |
| `thesmsworks_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `thesmsworks_list` and
`thesmsworks_load` tools become available in new sessions.
