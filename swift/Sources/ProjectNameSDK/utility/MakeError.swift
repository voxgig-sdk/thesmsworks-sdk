// Thesmsworks SDK utility: makeError - the single error surface of the
// pipeline. Throws the wrapped ThesmsworksError unless the per-call ctrl
...
func makeErrorUtil(_ ctx: Context, _ err: Error?) throws -> Value {
  ...
  var msg = "ThesmsworksSDK: " + opname + ": " + errmsg
  ...
  let code = (e as? ThesmsworksError)?.code ?? ""
  let sdkErr = ThesmsworksError(code, msg, ctx)
