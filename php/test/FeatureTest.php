...
class FtCtrl extends ThesmsworksControl
{
...
class FtClient
{
    ...
    private ThesmsworksUtility $utility;

    public function __construct(ThesmsworksUtility $utility, array $options)
    {
    ...
    public function get_utility(): ThesmsworksUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ThesmsworksContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ThesmsworksContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ThesmsworksContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ThesmsworksConfig::make_feature($name);
    ...
    private static function buildUrl(ThesmsworksSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ThesmsworksError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ThesmsworksResponse($fetched);
        }
    ...
    private function fail(ThesmsworksContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ThesmsworksContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
