...
class BatchEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set THESMSWORKS_TEST_BATCH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $batch_ref01_data = null;
        if (count($batch_ref01_data_raw) > 0) {
...
function batch_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("THESMSWORKS_TEST_BATCH_ENTID");
